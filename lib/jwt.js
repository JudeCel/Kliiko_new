...
let ACCOUNT_USER_ATTRS = [
  ...
  'landlineNumberCountryData', 'AccountId', 'gender','mobile', 'landlineNumber', 'owner',
...
const loadResources = (payloade) => {
  ...
  return new Bluebird((resolve, reject) => {
    ...
    resourceRepo(id).then((user) => {
      if (user) {
        responseObject.user = _.pick(user.dataValues, constants.safeUserParams)
        user.AccountUsers.sort((au)=> !au.owner);
