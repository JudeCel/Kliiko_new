...
const {SessionMember, AccountUser, User, Account} = require('./../models');
...
let ACCOUNT_USER_ATTRS = [
  ...
  'companyName', 'country', 'postCode', 'state', 'city', 'postalAddress'
  ];
    ...
    resourceRepo(id).then((user) => {
      if (user) {
        ...
        if(user.AccountUsers[0]){
          ...
          if(underAccount) {
            responseObject.account =  _.pick(user.AccountUsers[0].Account.dataValues, ["id", "name", "subdomain"])
          }
        }
...
const userReource = (id) => {
  return User.find({
    ...
    include: [{
      ...
      include: [{ model: Account, attributes: ["id", "name", "subdomain"]}]
    }]
  })
}
