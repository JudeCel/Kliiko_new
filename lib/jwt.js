...
let ACCOUNT_USER_ATTRS = ['id', 'role', 'firstName', 'lastName', 'email', 'phoneCountryData', 'landlineNumberCountryData', 'AccountId'];
    ...
    resourceRepo(id).then((user) => {
      if (user) {
        ...
        if(user.AccountUsers[0]){
          ...
          if(underAccount) {
            responseObject.account =  _.pick(user.AccountUsers[0].Account.dataValues, ["id", "name"])
          }
        }
