...
Raphael.fn.freeTransform = function(subject, options, callback) {
	...
	ft.padding = 10;								//	how much padding offset the box around text to give...
	ft.whiteboardMode = WHITEBOARD_MODE_NONE;	//	WHITEBOARD_MODE_* is defined in main.js
	...
	ft.updateHandles = function() {
		...
		//	TODO:	this is where we update the handle
		if ( ft.handles.center ) {
			var type = "undefined";
			if (typeof ft.subject != "undefined") {
				if (typeof ft.subject.type != "undefined") {
					type = ft.subject.type;
				}
			}
			...
			switch(type) {
				...
				case 'set': {
					...
					for (var ndxCorners = 0; ndxCorners < 4; ndxCorners++) {
						path = path + cmd + corners[ndxCorners].x + "," + corners[ndxCorners].y;
						cmd = "L"; 
					}

					var fill = '#7f7f7f';	//	default colour
					...
					ft.handles.center.disc
						.attr({path: path})
						.attr(ft.opts.attrs)
						.attr({'fill': fill})
						.toFront();

					// var tx = (ft.attrs.translate.x - ft.attrs.transX),
					// 	ty = (ft.attrs.translate.y - ft.attrs.transY);
					// var tx = /*ft.attrs.x +*/ ft.attrs.translate.x,
					// 	ty = /*ft.attrs.y +*/ ft.attrs.translate.y;

					//ft.handles.center.disc.toFront().transform("t" + tx + "," + ty);

					// ft.handles.center.disc.toFront().attr({
					// 	x: ft.attrs.x + ft.attrs.translate.x - (ft.padding / 2),
					// 	y: ft.attrs.y + ft.attrs.translate.y - (ft.padding / 2)
					// 	});
				}
				break;
				default: {
	...
	/*
		DKay - 2013/04/16
		whiteboardMode: int				//	WHITEBOARD_MODE_NONE | WHITEBOARD_MODE_MOVE | WHITEBOARD_MODE_DELETE
										//	please note, WHITEBOARD_MODE_NONE and WHITEBOARD_MODE_MOVE are treated the same
	*/
	ft.showHandles = function(whiteboardMode) {
		//	set up the default for isDelete
		if (typeof whiteboardMode === "undefined") whiteboardMode = WHITEBOARD_MODE_MOVE;
		ft.whiteboardMode = whiteboardMode;
		...
		//	we draw the handles here
		//	not sure where we update when the handle is dragged
		if ( ft.opts.drag.indexOf('center') !== -1 ) {
			...
			switch(type) {
				case 'text': 
				case 'set': {
					ft.attrs.transX = ft.attrs.translate.x;
					ft.attrs.transY = ft.attrs.translate.y;
					...
					// var bbox = null,
					// 	scaleX = (ft.attrs.scale.x || 1),
					// 	scaleY = (ft.attrs.scale.y || 1),
					// 	rotate = (ft.attrs.rotate || 0);

					//var transform = "r" + rotate + "s" + scaleX + ", " + scaleY;
					//var transform = "r" + rotate + "s" + 1 + ", " + 1;

					// if (ft.items.length === 2) {
					// 	var itemAttr = ft.items[(ft.items.length - 1)].el.attrs;
					// 	bbox = {
					// 		x: itemAttr.x,
					// 		y: itemAttr.y,
					...
					// 	};
					// } else {
					// 	bbox = (type === 'text') ? ft.subject[0].getBBox() : ft.subject[1].getBBox();
					// }
					...
					// if (ft.padding > 0) {
					// 	bbox.x = bbox.x - (ft.padding / 2);
					// 	bbox.y = bbox.y - (ft.padding / 2);
					// 	bbox.height = bbox.height + (ft.padding * 1);
					// 	bbox.width = bbox.width + (ft.padding * 1);
					// }
