/*
iiif ( (if (( (if ((((( (f (jqFilter  jQuery jqGrid filter addon.
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		*/
		...
		if (ble.append(tr);
			...
			var str= "", selected;
			for (i if (; i < p.groupOps.length; i++) {
	...
	if (	that.reDraw(); // the html has changed, force reDraw

					that.onchange(); // signals that the filter has changed
					return false;
				});
			if (			th.append(inputAddSubgroup);
			if(this.p.ruleButtons === true) {
			...
			if (ar searchable = (typeof that.p.columns[i].search === 'undefined') ?  true: that.p.columns[i].search ,
		...
		if (ar opr;
			...
			if (parentgroup !== null) { // ignore the first group
				var inif (DeleteGroup = $("<input type='button' value='-' title='Delete group' class='delete-group'/>");
	...
	if (	that.reDraw(); // the html has changed, force reDraw
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			if (r elm = $.jgrid.createEl(cm.inputtype,cm.searchoptions, "", true, that.p.ajaxSelectOptions, true);
			...
			for ( i = 0; i < op.length; i++) {
				ina = $.inArray(op[i],aoprs);
				if(ina !== -1) {
			if (elected = rule.op === that.p.ops[ina].name ? " selected='selected'" : "";
		...
		if (hat.onchange(); // signals that the filter has changed
				returif (alse;
			});
			}
			return tr;
	...
	if (unction getStringForGroup(group) {
				var s = "(", index;
