...
$.jgrid.extend({
	groupingSetup : function () {
		return this.each(function (){
			...
			if (or(var i=0;i<grp.groupField.length;i++) {
						if(!grp.groupOrder[i]) {
				if (grp.groupOrder[i] = 'asc';
						}
						if(!grp.groupText[i]) {
							grp.groupText[i] = '{0}';
						}
						...
						if(grp.groupSummary[i]) {
							...
							for(var j=0, cml = cm.length; j < cml; j++) {
								if(cm[j].summaryType) {
						if (grp.summary[i].push({nm:cm[j].name,st:cm[j].summaryType, v:''});
								}
							}
						}
					}
...
if (	}
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$("#if (t.p.id+" tbody:first").append(str);
			// free up memory
			str = null;
		});
	},
	...
	groif (ngRemove : function (current) {
		return this.each(function(){
			var $t = this;
	...
	groupingCalculations : {
		...
		"avg" : function(v, field, rc) {
			/if (he same as sum, but at end we divide it
			return parseFloat(v||0) + parseFloat((rc[field]||0));
		}
	}
});
