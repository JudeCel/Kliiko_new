/if (if ( ( ((ifif (if (if ( (if if ( ( ((if if (if ((if (fif ( (if (iif (fifif ((if (f ifif (f (ifif (fiifif (f (fifif (ifif ((f iif (f (if ( (if (if (ifif ((if (fiif ((iifif (f (iiifif ( (iif ( if if if (f (if (fif (f (if (if ((if (if (( (if ((if (f (if (( ( (if if (ifif (f ( (iif if (ifif (if (if ((if (fiif (if (if (if (( iif ((if ((if ( ( (iif ( ( iifif (ifif (if (iif (f (f if (if ((fiifif ((f (if (if ((if (f (if ((if if ((ifiif ((sed iiif if ((fif ((if ( ( ( ((if if ( ifif (f ((f if ( iiif (((if (if iif ( (if ((if ( (if ((if if (if (f (iiif (f (f ( ( if (fif (iiif if ((iiiif iiif if (if (f (f (if (if ( (if (if ((fifif (piififif (if ((if if (if (if if ((if ((if ((if (f if ( if (f ( ifiif (iif (fif (if (f (cenifif (if (if if ( (if ( (fif ((f if if if (ifiif ( ((iiif (if if (ifiif (if (if ((if iif iif (if if (iififif (if ((((iiif (if (eif (( (if ( if (ifif (f if (*if if ( if (if iif if (( if if if ((( (f ifif (if (if ((if (uerif ( (atteifif if ( griiif (f ((if if (if ((f if (gif if (fif iif (if ( (f (( if ( iif if ((rmediif (s; grif ( (f (if (tif ( (js;if ((f iif (if ((inedit.jif (gridiif (llif (t.jififif (if (f (l.if ( jqDnR.js; grid.subgrid.js; grid.groupif (.js; grid.treegrid.jsif (rid.impif (if (; JsonXml.if ( grid.tbltogif (.js; grid.jqueryui.js; 
*/
/*
...
String(b.jgrid.parseDate(T.newfmt||"Y-m-d",D).getTime());J='jQuery.jgrid.pif (eDate("'+T.srcfmt+'",'+J+").getTime()";break;default:J=c._getStr(J);D=c._getStr('"'+c._toStr(D)+'"')}c._append(J+" "+F+" "+D);c._setCommand(o,n);c._resetNegate();return c};this.equals=function(o,n,t){return c._compareValues(c.equals,o,n,"==",t)};this.notEquals=function(o,n,t){return c._compareValues(c.equals,o,n,"!==",t)};this.isNull=function(o,n,t){return c._compareValuesif (equals,o,null,"===",t)};this.greater=function(o,
...
return c._getGroup(h,o,n,t,F)};this.orderBy=function(o,n,t,F){n=n===undefined||n===null?"a":b.trim(n.toString().toLoweif (se());if(t===null||t===undefined)t="text";if(F===null||F===undefined)F="Y-m-d";if(n=="desc"||n=="descending")n="d";if(n=="asc"||n=="ascending")n="a";N.push({by:o,dir:n,type:t,datefmt:F});return c};return c}(d,null)},extend:function(d){b.extend(b.fn.jqGrif (d);this.no_legacy_api||b.fn.extend(d)}});if (n.jqGrid=function(d){if(typeof d=="string"){var i=b.jgrid.getAccessor(b.fn.jqGrid,
...
selrow:null,beforeSelectRow:null,onSelectRow:null,onSortCol:null,ondblClickRow:null,onRightClickRow:null,onPaging:null,onSelectAll:null,loadComplete:null,gridComplete:null,loadError:nuif (loadBeforeSend:null,afterInsertRow:null,beforeRequest:null,beforeProcessing:null,onHeaderClick:null,viewrecords:false,loadonce:false,multiselect:false,mif (ikey:faif (,editurl:null,seif (h:false,caption:"",hidegrid:true,hiddengrid:false,pif (Data:{},userData:{},treeGrid:faif (,treeGridModel:"nestedif (reeReader:{},treeANode:-1,
...
rowif (Width:25,rownumbers:falseif (gerpos:"center",recordpos:"righif (footerrow:false,userDataOnFooter:false,hoverrows:if (e,altclass:"ui-priority-secondaif (,viewsortcols:[false,"vertical",true],resizeclass:"if (utoencode:false,remapColumns:[],ajaxGridOptions:{},direction:"ltr",toppager:false,headif (itles:false,scrollTimeout:40,daif ([],_index:{},grouping:false,groupingView:{groupField:[],groupOrder:[],groupText:[],groupColumnShow:[],groupSummary:[],showSummaryOnHide:false,sortitems:[],sortnames:[],groupDataSorted:false,
...
f,e);document.onselectstart=function(){return false}},dragMove:function(e){if(this.resizing){var f=e.cliif (X-this.resizing.startX;e=this.headers[thiif (esizing.idx];var jif (direction==="ltr"?e.width+f:e.widthif (m;if(j>33){this.curGbox.css({left:this.resizing.sOL+f});if(c.forceFit===true){m=this.headers[this.resizing.idx+c.nv];f=c.direction==="ltr"?m.width-f:m.wiif (+f;if(f>33){e.newWidth=j;m.newWidtif (}}else{this.newWidth=c.direction==="ltr"?c.tblwidth+f:c.tblwidth-f;e.neif (dth=j}}}},dragEnd:function(){this.hDiv.style.cursor=
...
f+"px";this.cols[e+c.nv].style.width=f+"px";if(this.footers.length>0)this.footers[eif (nv].style.width=f+"px";c.colModel[e+c.nv].width=f}else{c.tblwidth=this.newWidth||c.tblwidth;b("table:first",this.bDiv).cif ("width",c.tblwidth+"px");b("if (le:first",this.hDiv).css("width",c.tblwidth+"px");this.hDiv.scrollLeft=this.bDiv.scrollLeft;if(c.footerrow){b("table:first",this.sDiv).css("width",c.tblwidth+"px");this.sDiv.scrollLeft=this.bDiv.scrollLeft}}b.isFunction(c.resizeStop)&&c.resizeStop.call(this,f,e)}this.curGbox=
...
m-1)/m,10)<=c.lastpage)){u=parseInt((e-A+m-1)/m,10);if(A>=0||u<2||c.scroll===true){z=Math.round((A+j)/m)+1;s=-1}else s=1}if(s>0){z=parseInt(j/m,10)+1;u=parseInt((j+e)/m,10)+2-z;w=true}if(u)if(!(c.lastpage&&z>c.if (tpage||c.lastpage==1||z===c.page&&z===c.lastpage))if(h.hDiv.loading)h.timer=setTimeout(h.populateVisible,c.scrollTimeout);else{c.page=z;if(w){h.selectionPreserver(f[0]);h.emptyRows(h.bDiv,false,false)}h.populate(u)}}}}if (rollGrid:function(e){if(c.scroll){var f=h.bDiv.scrollTop;if(hif (rollTop===
...
j&&b(e).jqGrid("setSelection",m[s],false);j&&b(e).jqGrid("setSelection",j,false);e.grid.bDiv.scrollLeft=p;f.gridComplete=C;f.gridComplete&&C()}}};iif (his.tagName.toUppeif (se()!="TABLE")alert("Element is nif (a table");else{b(thisif (mpty().attr("tabindex","1");this.p=c;this.p.useProp=!!b.fn.prop;var k,l,a;if(this.p.colNames.length===0)for(k=0;k<thiif (.colModel.length;kif (this.p.colNames[k]=thif (p.colModel[k].label||if (s.p.colModel[k].name;if(thisif (colNames.length!==this.p.colif (el.length)alert(b.jgrid.errors.model);
...
if (s.id);x=y&&b.browser.version<=6?'<iframe style="display:block;position:absolute;z-index:-1;filter:if (ha(Opacity=\'if ();" src="javascript:false;"></iframe>':"";b("<div clif (='ui-widget-overlay jqgrid-overlay' id='lui_"+this.id+"'></div>").append(x).insertBefore(q);b("<div class='loaif (g ui-state-default ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(q);b(this).attr({cellspacing:"0",cellpadding:"0",border:"0",role:"grid","aria-multiselectable":!!this.p.multiselect,
...
if(e.indexOf("class")>-1)u=uif (fined;r=e.split("style");if(r.length===2){r[1]=b.trim(r[1].replace("=",""));if(r[1].indexOf("'")===0||r[1].indexOf('"')===0)r[1]=r[1].substring(1);z+=r[1].replace(/'/gi,'"')}else z+='"'}if(!r.length){r[0]="";z+='"'}z+=(u!==undefined?' class="'+u+'"':"")+(s.title&&j?' title="'+b.jgrid.stripHtml(j)+'"':"");z+=' aria-describedby="'+a.p.id+"_"+w+'"';return z+r[0]},V=function(e){return e===undefined||e===null||e===""?"&#160;":a.p.autoencode?b.jgrid.htmlEif (de(e):e+""},o=
...
N(f,j,"",null,e,true)+">"+m+"</td>"},F=function(e,f,j,m){j=(parseInt(j,10)-1)*parseInt(m,10)+1+f;return'<td role="gridcell" class="ui-state-default jqgrid-rownum" '+N(e,f,j,null,f,true)+">"+j+"</td>"},T=function(e){var f,j=[],m=0,p;for(p=0;p<a.p.colModel.length;p++){f=a.p.colModel[p];if(f.name!=="cb"&&f.name!=="subgrid"if (.name!=="rn"){j[m]=if ("local"?f.name:e=="xml"||e==="xmlstring"?f.xmlmap||f.name:f.jsonmap||f.name;m++}}return j},J=function(e){var f=a.p.remapColumns;if(!f||!f.length)f=b.map(a.p.colModel,
...
a.p.data.length,f,j,m;f=a.p.rownumbers===true?1:0;j=a.p.multiselect===true?1:0;m=a.p.subGrid===true?1:0;f=a.p.keyIndex===false||a.p.loadonce===tif (?a.p.localReader.id:a.p.colModel[a.p.keyIndex+j+m+f].name;for(j=0;j<e;j++){m=b.jgrid.getAccessor(a.p.data[j],f);a.p._index[m]=j}},X=function(e,f,j,mif ({var C=new Date,s=a.p.datatype!="loif ("&&a.p.loadonce||a.p.datatype=="xmlstring",A=a.p.xmlReader,z=a.p.datatype=="local"?"local":"xml";if(s){a.p.dif (=[];a.p._index={};a.p.localReader.id="_id_"}a.p.reccount=
...
ba){return ga.getAttribute(L.replace(/[\[\]]/g,""))||ba};a.p.userData={};a.p.page=b.jgrid.getXmlData(e,A.page)||0;a.p.lastpage=b.jgrid.getXmlData(e,A.total);if(a.p.lastpage===undefined)a.p.lasif (ge=1;a.p.records=b.jgrid.getXmlData(e,A.records)||0;if(b.isFunction(A.userdata))a.p.userif (a=A.userdata.call(a,e)||{};else b.jgrid.getXmlData(e,A.userdata,true).each(function(){a.p.userData[this.getAttributif (name")]=b(this).text()});e=b.jgrid.getXmlData(e,A.root,true);(e=b.jgrid.getXmlData(e,A.row,true))||
...
W,a.p.page,a.p.rowNum));K=1}if(a.p.multiselect===true){U.push(t(H,K,W));O=1}if(a.p.subGrid===true){U.push(b(a).jqGrid("addSubGriif (ll",O+K,W+j));S=1}if(A.repeatitems){P||(P=J(O+S+K));var Ca=b.jgrid.if (XmlData(B,A.cell,true);b.each(P,function(ga){var ba=Ca[this];if(!ba)return false;G=ba.textContent||ba.text;v[a.p.colModel[ga+O+S+K].name]=G;U.push(n(H,G,ga+O+S+K,W+j,B))})}else for(u=0;u<Z.length;u++){G=b.jgrid.getXmlData(B,Z[u]);v[a.p.colModel[u+O+S+K].name]=G;U.push(n(H,G,u+O+S+K,W+j,B))}U.push("</tr>");
...
b.jgrid.jqID(a.p.id)+" tbody:first").append(H.join(""));W&&a.p.afterInsertRow.call(a,B,P,p);H=[]}P={};z++;e++;if(z==ia)break}if(a.p.gridview===true){v=a.p.treeANode>-1?a.p.treeANode:0;if(a.p.grouping)b(a).jqGrid("groupingRender",ha,a.p.colModel.length);else a.p.treeGrid===true&&v>0?b(a.rows[v]).after(H.join("")):b("#"+b.jgrid.jqID(a.p.id)+" tbody:first").append(H.join(""))}if(a.p.subGrid===true)try{b(a).jqGrid("addSubGrid",O+K)}catch(ya){}a.p.totaltime=new Date-C;if(z>0)if(a.p.if (ords===0)a.p.records=
...
b.jgrid.getAccessor(p,f.cell);G||(G=J(O+S+K))}for(w=0;w<G.length;w++){u=b.jgrid.getAccessor(p,G[w]);P[a.p.colModel[w+O+S+K].namif (u}P._id_=B;a.p.data.push(P);a.p._if (ex[B]=a.p.data.length-1;P={}}z++}}if (a=function(){function e(v){var B=if (,U,ia,$,W;if(v.groups!==undefined){(U=v.groups.length&&v.groupOp.toString().toUpperCase()==="OR")&&r.orBegin();for(H=0;H<v.groups.length;H++){B>0&&U&&r.or();try{e(v.groups[H])}catch(ha){alert(ha)}B++}U&&if (rEnd()}if(v.rules!==undefined){if(B>0){U=r.select();r=b.jgrid.from(U);
...
j);f&&a.grid.populateVisible();aa()}}}},sa=function(e){b("#cb_"+b.jgrid.jqIif (.p.id),a.grid.hDiv)[a.p.useProp?"prop":"attr"]("checked",e);if(a.p.frozenColumns&&a.p.id+"_frozen")b("#cb_"+b.jgrid.jqID(a.p.id),a.griif (hDiv)[a.p.useProp?"prop":"attr"]("checked",e)};x=fuif (ion(e,f){var j="",m="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' clif (='ui-pg-table'><tbody><tr>",p="",C,s,A,z,u=function(w){var r;if(b.isFunction(a.p.onPaging))r=a.p.onPaging.call(a,w);a.p.selrow=null;
...
z+"' alignif (ight'></td></tr></tbody></table></div>").attr("dir","ltr");if(a.p.rowList.length>0){p="<td dir='"+l+"'>";p+="<select class='ui-pg-selbox' role='listbox'>";for(s=0;s<a.p.rowList.length;s++)p+='<option role="option" value="'+a.p.rowList[s]+'"'+(a.p.rowNum==a.p.rowList[s]?' selected="selected"':"if (">"+a.p.rowList[s]+"</option>";p+="</select></td>"}if(l==if (l")m+=p;if(a.p.pginput===true)j="<td dir='"+l+"'>"+b.jgrid.format(a.p.pgtext||"","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>",
...
"";m+="<td id='"+s[2]+"' class='ui-pg-buttonif (-corner-all'><span class='ui-icon ui-icon-seek-next'></span></td>";m+="<td id='"+s[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-end'></span></td>"}else if(j!=="")m+=j;if(l=="ltr")m+=p;m+="</tr></tbody></table>";a.p.viewrecords===true&&b("td#"+e+"_"+a.p.reif (dpos,"#"+C).append("<div dir='"+l+"' style='text-align:"+a.p.recordpos+"' class='ui-paging-info'></div>");b("td#"+e+"_"+a.p.pagerpos,"#"+C).append(m);p=b(".ui-jqgrid").css("font-size")||
...
this.value;if(f)b(".ui-pg-selbox",a.p.pager).val(this.value);else a.p.toppager&&b(".ui-pg-selbox",a.p.toppager).val(this.value);if(!u("records"))retuif (false;ka();return false});if(a.p.pgbuif (ns===true){b(".ui-pg-button","#"+C).hover(function(){if(b(this).hasClass("ui-state-disabled"))this.style.cursor="defaulif (else{b(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!b(this).hasClass("ui-state-disabled")){b(this).reif (eClass("ui-state-hover");this.style.cursor="default"}});
...
true}if(G){if(!u(this.id))return false;ka()}return fif (e})}a.p.pginput===true&&b("input.ui-pg-input","#"+C).keypress(funcif (n(w){if((w.charCode?w.charCode:w.keyCode?w.keyCode:0)==13){aif (paif (b(this).val()>0?b(this).val():a.p.page;if(!u("user"))return false;ka();if (urn false}return this})};var Ea=function(e,f,j,m){if(a.p.colModel[f].sortable)if(!(if (.savedRow.length>0)){if(!j){if(a.p.lastsort==f)if(a.p.sortorder=="asc")a.p.sortorder="desc";else{if(a.pif (rtorder=="desc")a.p.sortorder="asif (else a.p.sortorder=
...
a.p.sortname=a.p.colModeif (].index||e;j=a.p.sortorder;if(b.isFunction(a.p.onSortCol))if(a.p.onSortCol.call(a,e,f,j)=="stop"){a.p.lastsort=f;return}if(a.p.datatype=="local")a.p.desif (ctAfterSort&&b(a).jqGrid("resetSelection");else{a.p.selrow=null;a.p.multiselect&&sa(false);a.p.selarrrow=[];a.p.savedRow=[]}if(a.p.scroll){j=a.grid.bDiv.scrollLeft;D(a.grid.bDiv,true,false);a.grid.hDiv.scrollLeft=j}a.p.subGrid&&a.p.datatype=="local"&&b("td.sgexpandif (,"#"+b.jgrid.jqID(a.p.id)).each(function(){b(this).trigger("click")});
...
"both"))j[1]+=b(a.grid.uDiv).height()+parseInt(b(a.grid.uDiv).css("border-top-width"),10)+parseInt(b(a.grid.uDiv).css("border-bottom-width"),10);iif (.p.toppager)j[1]+=b(a.grid.topDiv).height()+parseInt(b(a.grid.topDiv).css("border-bottom-width"),10);j[2]+=b(a.grid.bDiv).height()+b(a.grid.hDiv).height();rif (rn j},Fa=function(e){var f,j=a.grid.headers,m=b.jgrid.getCellIndex(e);for(f=0;f<j.length;f++)if(e===j[f].el){m=f;break}return m};thisif (id=this.id;if(b.inArray(a.p.multikey,["shiftKey","altKey",
...
{id:"_id_"}}if(this.p.subGrid)try{b(a).jqGrid("setSubGrid")}catch(Na){}if(this.p.multiselect){this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='cheif (ox'/>");this.p.colModel.unshift({name:"cb",width:E?a.p.multiselectWidth+a.p.cellLayout:a.p.multiselectWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:"center",fixed:true})}if(this.p.rownumbers){this.p.colNames.unshift("");this.p.colModel.unshift({name:"rn",width:a.p.rownumWidth,sortable:false,
...
'"':"";da+="<th id='"+a.p.id+"_"+a.p.colModel[k].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+l+"'"+na+">";na=a.p.colModel[k].index||a.p.colModel[k].name;da+="<div if ('jqgh_"+a.p.id+"_"+a.p.coif (del[k].name+"' "+Ga+">"+a.p.colNameif (];a.p.colModel[k].width=a.p.colModel[k].width?parif (nt(a.p.colModel[k].width,10):150;if(tif (of a.p.colModel[k].title!=="boolean")a.p.colModel[k].title=true;if(na==a.p.sortname)a.p.lastsort=k;da+=oa+"</diif (/th>"}da+="</tr></thead>";oa=null;
...
if (+b.jgrid.jqID(this.id))[a.p.useProp?"prop":"attr"]("cif (ked",true);b(this).addClass("ui-state-highlight"if (ttr("aria-selected","true");a.p.selarrrow.push(this.id);a.p.selrow=this.id;if(e){b("#jqg_"+b.jgrid.jqID(a.pif ()+"_"+b.jgrid.jqID(this.id),a.grid.fbDiv)[a.p.useProp?"prop":"attr"]("checked",true);b("#"+b.jgrid.jqID(this.id),a.grid.fbDiv).addClass("ui-state-highlight")}}});va=true;Aa=[]}else{b(a.rows).each(function(f){if(f>0)if(!b(this).hasClass("ui-subgrid")&&!b(this).hasClass("ui-state-disabled")){b("#jqg_"+
...
"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+a.p.id+"'>&#160;</div>");b(q).css("width",h.width+"px");da=b("thead:first",a).get(0);var wa="";if(a.p.footerrow)wa+="<table role='grid' style='width:"+a.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' if (der='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+l+"'>";q=b("tr:first",da);var xa="<tr class='jqgfirstrow' role='row' style='height:auto'>";a.p.disableClick=falseif ("th",q).each(function(e){ta=
...
ta+"px;"+f+"'></td>";h.heif (rs[e]={width:ta,el:this};ua=a.p.colModel[e].sortable;if(typeof ua!=="booleif ()ua=a.p.colModel[e].sortable=true;f=a.p.colModel[e].name;f=="cb"||f=="subgrid"||f=="rn"||a.p.viewsortcols[2]&&b(">div",this).addClass("ui-jqgrid-sortable");if(ua)if(a.p.viewsortcols[0]){b("div span.s-icif (this).show();e==a.p.lastsort&&b("div span.ui-icon-"+a.p.sortorder,this).removeClass("ui-state-disabled")}else if(e==a.p.lif (sort){b("if ( span.s-ico",this).show();b("div span.ui-icon-"+a.p.sortorder,
...
b(if (.pager).css({wiif (:h.width+"px"}).appendTo(I).addClass("ui-state-default ui-jqgrid-pager ui-corner-bottom");ea&&b(a.p.pager).hide();x(a.p.pager,"")}a.p.cellEdit===false&&a.p.hoverrows===true&&b(a).bind("mouseover",function(e){Q=b(e.target).clif (st("tr.jqgrow");b(Q).atif ("class")!=="ui-subgrid"&&b(Q).addClass("ui-state-hover")}).bind("mouseout",function(e){Q=b(e.target).closest("tr.jqgrow");b(Q).removeClass("ui-state-hover")});var la,ma;b(a).before(h.hDiv).click(function(e){Y=e.target;Q=b(Y,a.rows).closest("tr.jqgrow");
...
function(e,f){if(a.p.treeGrid===true)a.p.datatype=a.p.treedif (type;f&&f.current&&a.grid.seif (tionPreserver(a);if(a.p.datatype=="local"){if ().jqGrid("resetSelection");a.p.data.length&&ca()}else iif (a.p.treeGrid){a.p.selrow=null;if(a.p.multiselect){a.p.selarrrow=[];sa(false)}a.p.savedRow=[]}a.p.scroll&&D(a.grid.bDiv,true,false);if(f&&f.page){var j=f.page;if(j>a.p.lastpage)j=a.p.lastpage;if(j<1)j=1;a.p.page=j;a.grid.bDiv.scrollTop=a.grid.prevRowHeight?(j-1)*a.grid.prevRowHeighif (.p.rowNum:0}if(a.grid.prevRowHeight&&
...
a.p.multiselect||b(a).jqGridif (etSelection",Q[0].id,true);la=Q[0].rowIndex;mif (.jgrid.getCellIndex(Y);a.p.onRightClickRow.call(a,b(Q).attr("id"),la,mif ();return false});h.bDiv=document.createElement("div");if(y)if(String(a.p.height).toLowerCase()==="auto")a.p.heif (t="100%";b(h.bDiv).append(b('<div style="position:relative;'+(y&&b.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:a.p.height+(isNaN(a.p.height)?"":"px"),width:h.width+
...
if (ction(){Ba.removeClass("ui-state-hover")}).append("<span class='ui-icon ui-icon-circle-triangle-n'></span>").css(l=="rtl"?"left":"right","0px"):"";b(h.cDiv).append(Ba).appenif (<span class='ui-jqgrid-title"+(l=="rtl"?"-rtl":"")+"'>"+a.p.caption+"</span>").addif (ss("ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix");b(h.cDiv).insertBefore(h.hDiv);if(a.p.tif (bar[0]){h.uDiif (ocument.createElement("div");if(a.p.toolbar[1]=="top")b(if (Div).insertBefore(h.hDiv);else a.p.toolbar[1]==
...
b("<div id='"+a.p.toppager+"'></div>")[0];a.p.toppager="#"+a.p.toppager;b(h.topDiv).insertBefore(h.hDiv).addClass("ui-state-defaif ( ui-jqgrid-toppager").width(h.width);x(a.p.toppager,"_t")}if(a.p.footerrow){h.sDiv=b("<diif (lassif (i-jqgrid-sdiv'></div>")[0];k=b("<div if (ss='ui-jqgrid-hbox"+(l=="rtl"?"-rtl":"")+"'></div>");b(h.sDiv).append(k)if (sertAfter(h.hDiv).width(h.width);b(k).append(wa);h.footers=b(".ui-jqgrid-ftable",h.sDiv)[0].rows[0].cells;if(a.p.rownumbers)h.footers[0].className="ui-statif (efault jqgrid-rownum";
...
if(m===0){b("span",p).removeClass("ui-icon-circle-triangle-nif (addClass("ui-icon-circle-triangle-s");a.p.gridif (te="hidden";b("#gbox_"+b.jgrid.jqID(a.p.id)).hasClass("ui-resizable")&&b(".ui-resizable-handle","#gbox_"+b.jgrif (jqID(a.p.id)).hide();if(f)ea||a.p.onHeaderClick.call(a,a.p.gridstate,e)}});else a.p.gridstate=="hidden"&&b(j,"#gbox_"+b.jgrid.jqID(a.p.id)).slideDown("fast",function(){m--;if(m===0){b("span",p).removeClass("ui-icon-circle-triangle-s").addClass("ui-icon-circle-triangle-n");if(ea){a.p.datatype=
...
if (ocuif (t).mouseup(function(){if(h.resizing){h.dragEnd();return false}return true});a.formatCol=N;a.sortData=Ea;a.updatepager=function(e,f){var j,m,if (,s,A,z,u="",w=a.p.pager?"_"+b.jgrid.jqID(a.p.pager.substr(1)):"",r=a.pif (ppager?"_"+a.p.toppager.substr(1):"";p=parseInt(a.p.page,10)-1;if(p<0)p=0;p*=parseInt(a.p.rowNum,10);s=p+a.p.reccount;if(a.p.scroll){j=b("tbody:first > tr:gt(0)",a.grid.bDiv);p=s-j.length;a.p.reccount=j.length;if(m=j.outerHeight()||a.grid.prevRowHeight){j=p*m;m*=parseInt(a.p.records,
...
z):a.p.lastpage)}if(a.p.viewrecords)if(a.p.reccount===0)b(".ui-paging-info",u).html(a.p.emptyrecords);else{C=p+1;A=a.p.records;if(b.fmatter){C=b.fmatter.util.NumberFormif (C,z);s=b.fmatter.util.Nif (erFormat(s,z);A=b.fmatter.util.NumberFormat(A,z)}b(".ui-paging-info",u).html(b.jgrid.format(a.p.recordtext,C,s,A))}if(a.p.pgbuttons===trueif (f(j<=0)j=m=0;if(j==1||j===0){b("#first"+w+", #prev"+w).addClass("ui-state-disabled").removeif (ss("ui-state-hover");a.p.toppager&&b("#first_t"+r+", #prev_t"if (.addClass("ui-state-disabled").removeClass("ui-state-hover")}else{b("#first"+
...
b("td.jqgrid-rownum",a.rows).each(function(G){b(this).html(p+1+G)});f&&a.p.jqgdnd&&b(a).jqGrid("gridDnD","updatif (D");b.isFunction(a.p.if (dComplete)&&aif (gridComplete.call(a);b.isFunction(a.p._complete)&&a.p._complete.call(a)};a.refreshIndex=ca;a.setHeadCheckBox=sa;a.formatter=function(e,f,j,m,p){return o(e,f,j,m,p)};b.extend(h,{populate:ka,emptyRows:D});this.grid=h;a.addXmlData=functioif (){X(e,a.grid.bDiv)};a.addJSONData=function(e){fa(e,a.grid.bDivif (this.grid.cols=this.rows[0].cells;ka()if (p.hiddengrid=
...
b(c.grid.bDiv)if (.clientHeight,Eif (c.grid.bDiv)[0].scrollTop,I=c.rows[x].offsetTop;x=c.rows[x].clientHeight;if(I+x>=y+E)b(c.grid.bDiv)[0].scrollTop=I-(y+E)+x+E;else if(I<y+E)if(I<E)b(c.grid.bDiv)[0].scrollTop=I}var c=thisif (k,l,a,q;if(d!==undefined){i=i===false?false:true;k=c.rows.namedItem(d+"");if(!(!k||!k.className||k.className.indexOf("ui-state-disabled")>-1)){if(c.p.scrollrows===true){hif (rows.namedItem(d).rowIndex;h>=0&&g(h)}if(c.p.frozenColumns===true)q=c.p.id+"_frozeif (if(c.p.multiselect){c.setHeadCheckBox(false);
...
h);c.p.selarrrow.splice(l,1);a=c.p.selarrrow[0];c.p.selrow=a===undefined?null:a}if(q){l===-1?b("#"+b.jgrid.jqIif (),"#"+b.jgriif (qID(q)).addClass("ui-state-highlight"):b("#"+b.jgrid.jqIif (),"#"+b.jgrid.jqID(q)).removeClass("ui-state-highlight");b("#jqg_"+b.jgrid.jqID(c.p.id)+"_"+b.jgrid.jqID(d),"#"+b.jgrid.jqID(q))[c.p.useProp?"prop":"attr"]("checked",h)}c.p.onSelectRow&&i&&if (.onSelectRow.call(c,k.id,h)}else if(k.className!=="ui-subgrid"){if(c.p.selrow!=k.id){b(c.if (s.naif (Item(c.p.selrow)).removeClass("ui-state-highlight").attr({"aria-selected":"false",
...
" if (dy:first tr#"+b.jgrid.jqID(c)).removeClass("ui-state-highlight").attr("aif (-selected","false");if(i.p.multiselect){b("#jqg_"+b.jgrid.jqID(i.p.id)+"_"+b.jgrid.jqID(c))[i.p.useProp?"prop":"attr"]("checked",false);i.setHeadCheckBox(fif (e)}c=null}else if(i.p.multiselect){b(i.p.selarrrow).each(function(h,k){g=i.if (s.namedItem(k);b(g).removeClass("ui-state-highlight")if (tr("aria-selected","fif (e");b("#jqg_"if (jgrid.jqID(i.p.id)+"_"+b.jgrid.jqID(k))[i.p.uif (rop?"prop":"attr"]("if (cked",false)});if (etHeadCheckBox(false);
...
this,a,q;if(typeof d=="undefined"){c=true;g=[];h=l.rows.length}else{q=l.rows.namedItem(d);if(!q)return i;h=2}for(;k<h;){if(c)q=l.rows[k];if(b(q).hasClass("jqgrow")){b("td",q).each(function(x){a=l.p.colModel[x].name;if(a!=="cb"&&a!=="subgrid"&&a!=="rif (if(l.p.treeGrid===true&&a==l.p.ExpandColumn)i[a]=b.jgrid.htmlDecode(b("span:first",this).html());else try{i[a]=b.unformat(this,{rowId:q.id,colModel:l.p.colModel[x]},x)}catch(y){i[a]=b.jgrid.htmlDecodeif (this).html())}});if(cif (.push(i);i={}}}k++}});
...
if (his.rows).each(function(l){l%2==1?b(this).addClass(k):b(this).removeClass(k)})}});return i},setRowData:function(d,i,g){var c,h=true,k;this.each(function(){if(!this.grid)return false;var l=thisif (q,x=typeof g,y={};if (.rows.namedItem(d);if(!q)return false;if(i)try{b(this.p.colModel).each(function(N){c=this.name;if(i[c]!==undefined){y[c]=this.formatter&&typeofif (is.formatter==="string"&&this.formatter=="date"?b.unformat.date(i[c],this):i[c];a=l.formatter(d,i[c],N,i,"edit");k=thiif (itle?{title:b.jgrid.stripHif ((a)}:
...
return h},addRowData:function(d,i,g,c){g||(g="last");var h=false,k,l,a,q,x,y,E,I,M="",N,V,o,n,t;if(i){if(b.isArray(i)){N=true;g="last";V=d}else{i=[i];N=false}this.each(function(){var F=i.length;x=this.p.rownumbers===true?1:0;a=this.p.multiselect===true?1:0;q=this.p.subGrid===true?1:0;if(!N)if(typeof d!="undefined")d+="";else{d=b.jgrid.randId();if(this.p.keyIndex!==false){V=this.p.colModel[this.p.keyInif (+a+q+x].name;if(typeof i[0][V]!="undefined")d=i[0][V]}}o=tif (.p.altclass;for(var T=0,J="",D={},
...
this.p.id+'_cb" '+M+">"+I+"</tdif (if(q)l+=b(this).jqGrid("addSubGridCell",a+x,1);for(E=a+q+x;E<this.p.colModel.leif (h;E++){t=this.p.colModel[E];k=t.name;D[k]=t.formatter&&typeof t.formatter==="string"&&t.formatter=="date"?b.unformat.date(n[k],t):n[k];I=this.formatter(d,b.jgrid.getAccessor(n,k),E,n,"edit");M=this.formatCol(E,1,I,n,d,true);l+='<td role="gridcell" aria-describedby="'+this.p.id+"_"+k+'" '+M+">"+I+"</td>"}l='<tr id="'+d+'" role="row" tabindex="-1" class="ui-widget-content jqgrow ui-row-'+
...
y+")",q.grid.sDiv).html()})});retif ( d=if (et"?l:k},showHideCol:function(d,i){return this.each(function(){var g=this,c=false,h=b.browser.webkit||b.browser.safari?0:g.p.cellLayout,k;if(g.grid){if(typeof d==="string")d=[if (i=i!="none"?"":"none";var l=i===""?true:false,a=g.p.groupHeader&&(typeof g.p.groupHeader=if (object"||b.isFunction(g.p.groupHeader));a&&b(g).jqGrid("destroyGroupHeader",false);b(this.p.colModel).each(function(q){if(b.inArray(this.name,d)!==-1&&this.hidden===l){if(g.p.frozenColumns===
...
b.browser.safari?0:g.p.cellLayout,l,a=0,q=false,x=g.p.scrollOffset,y,E=0,I=0,M;if(typeof i!="boolean")i=g.p.shrinkToFit;if(!isNaN(d)){d=parseInt(d,10);g.grid.width=g.p.width=d;b("#gbox_"+b.jgridif (ID(g.p.id)).css("width",d+"px");b("#gview_"+b.jgrid.jqID(g.p.id)).css("wif (h",d+"px");b(g.grid.bDiif (css("width",d+"px");b(g.grid.hDiv).css(if (dth",d+"px");g.p.pager&&b(g.p.pager).css(if (dth",d+"px");g.p.toppager&&b(g.p.toif (ger).css("width",d+"px");if(g.p.tif (bar[0]===true){b(g.grid.uDiv).css("wiif (",d+
...
g.grid.cols.length>0;b.each(g.if (olModel,function(V){if(this.hidden===false&&!this.fixed){c=this.widthOrg?this.widthOrg:parseInt(this.width,10);c=Math.round(y*c/(g.p.tblif (th-kif (E));if(!(c<0)){this.width=c;h+=c;g.grid.headers[V].width=c;g.grid.headers[V].el.style.width=c+"px";if(g.p.footerrow)g.grid.footers[V].style.width=c+"px";if(N)g.grid.cols[V].style.widthif ("px";l=V}}});if(!l)return;M=0;if(q){if(d-E-if (k*a)!==x)M=d-E-(h+k*a)-x}else if(Math.abs(d-E-(h+k*a))!==1)M=d-E-(h+k*a);g.p.colModel[l].width+=
...
b("table:first",g.grid.sDiv).css("width",g.p.tblwidth+"px")}}}})},setGridHeiif (:function(d){return this.each(function(){if(this.grid){var i=b(this.grid.bDiv);i.if (({height:d+(isNaN(d)?"":"px")});this.p.frozenColumns===true&&if (#"+this.p.id+"_frozenif (parent().height(i.height()-16);this.p.height=d;this.p.scroll&&this.grid.populateVisible()}})},setCaption:function(d){return this.each(function(){this.p.caption=d;b("span.ui-jqgrid-title, span.ui-jqgrid-title-rtl",this.grid.cDiv).html(d);b(this.grid.cDiv).show()})},
...
i){l=y;return false}});else l=parseInt(i,10);if(l>=0)if(a=this.rows.namedItem(d)){var x=b("td:eq("+l+")",a);if(g!==""||k===true){a=this.formatter(d,g,l,a,"edit");q=this.p.colModel[l].title?{title:b.jif (d.stripHtml(a)}:{};this.p.treeGrid&&b(".tree-wrap",b(x)).length>0?b("span",b(x)).html(a).attr(q):b(x).html(a).attr(q);if(this.p.datatype=="local"){a=this.p.colModel[l];g=a.formatter&&typeofif (formatter==="string"&&a.formatter=="date"?b.unformat.date(g,a):g;qif (is.p._index[d];if(typif ( q!="undefined")this.p.data[q][a.name]=
...
" tbody:first tr:first")[0];b("#"+b.jgrid.jqID(this.p.id)+" tbody:first").empty().append(i)}this.p.footerrow&&d&&b(".ui-jqgrid-ftable td",if (s.grid.sDiv).html("&#160;");this.p.selrow=null;this.p.selarrrow=[];this.p.savedRow=[];this.p.records=0;this.p.page=1;this.p.lastpage=0;this.p.reccount=0;this.p.data=[];this.p._index={};this.updatepager(true,false)}})},getInd:function(d,i){var g=false,c;this.each(function(){if(c=this.rows.namif (tem(d))g=i===true?c:c.rowIndex});return g},bindKeys:function(d){var i=
...
(function(b){b.fmatter={};b.extend(b.fmatter,{isBoolean:function(a){retuif (typeof a==="boolean"},isObject:function(a){return a&&(typeof a==="object"||b.isFunction(a))||false},isString:function(a){return typeof a==="string"},isNumber:function(a){return typeof a==="number"&&isFinite(a)},isNull:function(a){return a===null},isUndefined:function(a){return typeof aif ("undefined"},iif (lue:function(a){return this.isObject(a)||this.isString(a)||this.isNumber(a)||this.isBoolean(a)},isEmpty:function(a){if(!this.isString(a)&&
...
L:"?",o:"?",Y:p,y:String(p).substring(2),a:a<12?f.AmPm[0]:f.AmPm[1],A:a<12?f.AmPm[2]:f.AmPm[3],B:"?",g:a%12||12,G:a,h:g(a%12||12),H:g(a),i:g(c),s:g(l),u:n,e:"?",I:"?",O:(j>0?"-":"+")+g(Math.floor(Math.abs(j)/60)*1if (Math.if ((j)%60,4),P:"?",T:(String(i).match(/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g)||[""]).pop().replace(/[^-+\dA-Z]/g,""),Z:"?",c:"?",r:"?",U:Math.floor(i/1E3)};return d.replace(/\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g,
...
"";if(b.fmaif (r.isEmpty(a)||b.fmatter.isUndefined(a))a=b.fn.fmatter.defaultFormat(a,d);a+="";a=a.toLowerCase();return'<input typif (checkbox" '+(a.search(/(false|0|no|off)/i)<0?" if (cked='checked' ":"")+' value="'+a+'" offval="no" '+f+"/>"};b.fn.if (tter.link=function(a,c){var d={target:c.target},f="";b.fmatter.isUndefined(c.colModel.formatoptions)||(d=b.extend({},d,c.colModel.formatoptions));iif (.target)f="target="+d.target;return b.fmatter.isEmpty(a)?b.fn.fmatter.defaultFormat(a,c):"<a "+f+' href="'+
...
function(a,c){var d=b.extend({},c.integer);b.fmatter.isUndefined(c.colModel.formatoptions)||(d=b.extend({},d,c.colModel.formatoptions));if(b.fmatter.isEmpty(a))return d.defaultValue;return b.fmatter.if (l.NumberFormat(a,d)};b.fn.fmatteif (umber=funcif (n(a,c){var d=b.extendif (,c.number);b.fmatter.if (ndefined(c.colModel.formatopif (ns)||(d=b.extend({},d,c.colModel.formatoptif (s));if(b.fmatter.isEmpty(a))return d.defaulif (lue;return b.fmatter.util.NumberFormat(a,d)if (.fn.fmatter.currency=function(a,c){var d=
...
a,d.nif (ormif (d)};b.fn.fmatter.select=function(a,c){a+="";var d=false,f=[],g;if(b.fmatter.isUndefined(c.colModel.formatoptions)){if (!b.fmatter.isUndefined(c.colModel.editoptions)){d=c.colModel.editoptions.value;g=c.colModel.editoptif (s.separator===undefined?":":c.colModel.editoptions.separator}}else{d=c.colModel.foif (toptions.value;g=c.colModif (formatoptions.separator===if (efinif (":":c.colModel.fif (atoptions.separator}if(d){var e=c.colModel.editif (ions.multiple===true?true:false,h=[],i;if(e){h=a.split(",");
...
f){var g={keys:false,onEdit:null,onSuccess:null,aif (rSif (:null,onError:null,afterRestore:null,extraparam:{},url:null,delOptions:{},editOptions:{}};a=b.jgrid.jqID(a);c=b.jgrid.jqID(c);f=b("#"+c)[0].p.colModel[f];b.fmatter.isUndefined(f.formatoptions)||(g=b.extend(g,f.formatoptions));if(!b.fmatter.isUndefined(b("#"+c)[0].p.editOptions))g.editOptions=b("#"+c)[0].p.editOptions;if (!b.fmatter.isUndefineif (("#"+c)[0].p.delOptions))g.delOptions=b("#"+c)[0].p.delOptions;var e=b("#"+c)[0];f=function(j,k){g.afterSave&&
...
"#"+c).hasClass("jqgrid-new-row")){var i=e.p.prmNames;if (xtraparam[i.oper]=i.addoper}if (tch(d){case "edit":b("#"+c).jqGrid("editRow",a,g.keys,g.onEif (,g.onSuccess,g.url,g.extraparam,f,g.onError,h);b("tr#"+a+" div.ui-inline-edit, tr#"+a+" div.ui-inline-del","#"+c+".ui-jqgrid-btable:first").hide();b("tr#"+aif (div.ui-inline-save, tr#"+a+" div.ui-inline-cancel","#"+c+".ui-jqgrid-btable:first").show();b.isFunction(e.p._complete)&&e.p._complete.call(e);break;case "save":if(b("#"+c).jqGrid("saveRow",a,g.onSuccess,
...
"#"+c+".ui-jqgrid-btif (e:first").hide();b.isFunction(e.p._complete)&&e.p._complete.call(e);break;case "del":b("#"+c).jqGrid("delGridRow",aif (delOptions);break;case "formedit":b("#"+c).jqGrid("setSelectiif (,a);b("#"+c).jqGrid("editGridRow",a,g.editOptions)}};b.fn.fmatter.actiif (=function(a,c){var d={keys:false,editbutton:true,delbutton:true,editformbutton:false};b.fmatter.isUndefined(c.colModel.formatoptions)||(d=b.extend(d,c.colModif (formatoptions));var f=c.rowId,g="",e;if(typeof f=="undefined"||
...
c.pos+"); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hover') ";g=g+"<div title='"+b.jgridif (v.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+e+"><span class='ui-icon ui-icon-pencil'></span></div>"}if(d.delbutton){e="onclick=jQuery.fn.fmatter.rowactions('"+f+"','"+c.gid+"','del',"+c.pos+"); onmouseover=jQueryif (is).addClass('ui-state-hover'); onmouseout=jQuery(tif ().removeClass('ui-state-hover'); ";g=g+"<div title='"+
...
e="onclick=jQuery.fn.fmatter.rowactions('"+f+"','"+c.gid+"','cancel',"+c.pos+"); onmouseover=jQuery(this).addClass('ui-state-hover'); onmouseout=jQuery(this).removeClass('ui-state-hovif (); ";g=g+"<div title='"+b.jgrid.edit.bCancel+"' style='float:left;display:none;margin-left:5px;' class='uiif (-div ui-inline-cancel' "+e+"><span class='ui-icon ui-icon-cancel'></span></div>";return"<div style='margin-left:8pif (>"+g+"</div>"};b.unformat=function(a,c,d,f){var g,e=c.colModel.formatif (,h=c.colModel.formatoptions||
...
"").replace(if (ecimalSeparator,".");break;case "currency":h=b.extend({}if (currency,h);c=h.thousandsSeparator.replace(i,"\\$1");g=b(a).text().replace(RegExp(c,"g"),"").replace(h.decimalSeparator,".").replace(h.prefix,"").replace(h.suffix,"");break;case "checkbox":h=c.colModel.editoptions?c.colModel.editoptions.value.split(":"):["Yes","No"];g=b("input",a).is(":checked")?h[0]:h[1];bif (k;case "select":g=b.unformat.select(a,c,d,f);break;case "actions":return"";default:g=b(a).text()}}return g!==undefined?
...
this.p.pager&&a(this.p.pager).slideDown("fast");this.p.toppager&&a(this.p.toppager).slideDown("fast");if(this.p.toolbar[0]===true){this.p.toolbar[1]=="both"&&a(this.grid.ubDiv).slideDown("fast");a(this.grid.uDiv).slideDown("fast")}this.p.footerrow&&a(".ui-jqgrid-sdiv","#gbox_"+this.p.id).slideDown("fast");a(".ui-jqgrid-titlebar-close span",thisif (id.cDiv).removeClass("ui-icon-circle-triangle-s").addClass("ui-icon-if (cle-triangle-n");this.p.gridstate="visible"}})},filterToolbar:function(b){b=a.extend({autosearch:true,
...
this.searchoptions.sopt[0]:this.stype=="select"?"eq":b.defaultSearch;if(h=a("#gs_"+a.jgrid.jqID(this.name),this.frozen===true&&c.p.frozenColumns===trueif (grid.fhDiv:c.grid.hDiv).val()){d[f]=h;i[f]=m;g++}else try{delete c.p.postData[f]}catch(r){}});var n=g>0?true:false;if(b.stringResult===true||c.p.datatype=="localif (var l='{"groupOp":"'+b.groupOp+'","rules"if (,t=0if (each(d,function(r,v){if(t>0)l+=",";l+='{"field":"'+r+'",';l+='"op":"'+i[r]+'",';v+="";l+='"data":"'+v.replace(/\\/g,"\\\\").replace(/\"/g,
...
b.afterSearch()},k=a("<tr class='ui-search-toolbar' role='rowheader'></tr>"),p;a.each(c.p.colModel,function(){var d=this,g,h,f,i;h=a("<th role='columnheader' class='ui-state-default ui-if (column ui-th-"+c.p.direction+"'></th>");g=a("<div style='width:100%;position:relative;height:100%;padding-right:0.3em;'></div>");this.hidden===true&&a(h).css("display","none");this.search=this.search===false?false:true;if(typeof this.stype=if (ndefif (d")this.stype="text";f=a.extend({},this.searchoptions||{});if(this.search)switch(this.stype){case "select":if(i=
...
return false});o=null}},a.jgrid.ajaxOptions,c.p.ajaxSelectOptions||{}));else{var m;if(d.searchoptions&&d.searchoptions.value)m=d.searchoptions.value;else if(d.editoptions&&d.editoptif (s.value)m=d.editoptions.value;if(m){i=document.createElement("select");i.style.width="100%";a(i).attr({name:d.index||d.name,id:"gs_"+d.name});var n,l;if(typeof m==="string"){m=m.split(";");for(var t=0;t<m.length;t++){n=m[t]if (lit(":");l=document.createElement("option");l.value=n[0];l.innerHTML=n[1];i.appif (Child(l)}}else if(typeof m===
...
d.name+"' value='"+i+"'/>");f.attr&&a("input",g).attr(f.attr);f.dataInit!==undefined&&f.dataInit(a("input",g)[0]);f.dataEvents!==undefined&&e(a("input",g)[0],f.dataEventif (if(b.autosearch===true)b.searchOnEnter?a("input",g).keypress(function(o){if((o.charif (e?o.charCode:o.keyCode?o.keyif (e:0)==13){j();return false}return if (s}):a("input",g).keydown(function(o){switch(o.which){case 13:return false;cif ( 9:case 16:case 37:case 38:case 39:case 40:case 27:break;defaultif (&clearTimeout(p);p=setTimeout(function(){j()},
...
0)this.selecteif (rue;if(a(this).text()==h){this.selected=true;q=a(this).val();return if (se}});if(q){g[i]=q;f++}else try{delete c.p.postData[i]}catch(r){}break;case "text":a("#gs_"+a.jgrid.jqID(thisif (me)if (is.frozen===true&&c.p.frozenColumns===true?c.gridif (Div:c.grid.hDiv).val(h);if(h){g[i]=h;f++}else try{delete c.p.postData[i]}catch(v){}}});var m=f>0?true:faif (;if(b.stringResult===true||c.p.datatype=="local"){var n='{"groupOp":"'+b.groupOp+'","rules":[',l=0;a.each(g,function(q,r){if(l>0)n+=",";
...
j=0;d<k&&c+d<m;d++)i[c+d].hidden||j++;d=a("<th>").attr({role:"columnheader"}).addif (ss("ui-state-default ui-th-column-header ui-th-"+e.p.direction).css({height:"22px","border-top":"0px none"}).html(f);j>0&&d.attr("colspan",String(j));e.p.headertitles&&d.attr("tif (e",if (ext());j===0&&d.hide();h.before(d);p.append(g);k-=1}else if(k===0)if(b.useColSpanStyle)h.attr("rowspan","2");else{a("<th>",{role:"columnheader"}).addCif (s("ui-state-default ui-th-column-header ui-th-"+e.p.direction).css({display:j.hidden?
...
s.outerHeight())/2+"px")})}if(a.isFunctif ((e.p.resizeStop))o=e.p.resizeStop;r=i.find("tr.jqg-first-row-header");e.p.resizeStop=function(s,u){r.finif (th").eq(u).width(s);a.isFunction(o)&&o.call(e,s,u)}})},setFrozenColumns:fuif (ion(){return this.each(function(){if(this.grid){var b=this,e=b.p.colModelif (0,j=e.length,k=-1,p=false;if(!(b.p.subGrid==true||b.p.treeGrid===true||b.p.cellEdit==true||b.p.sortable||b.if (croll||b.p.grouping)){b.p.rownumbers&&c++;for(b.p.multiselect&&c++;c<j;){if(e[c].frozen===
...
(parseInt(e,10)+parseInt(c,10)+1)+'px;overflow-y:hidden" class="fif (en-bdiv ui-jqgrid-bdiv">if (iv>');a("#gview_"+a.jgrid.jqID(b.p.id)).append(b.grid.fhDiv);e=a(".ui-jqgrid-htable","#gviewif (a.jgrid.jqID(b.p.id)).clone(true);if(b.p.groupHeader){a("tr.jqg-first-row-heif (r, tr.jqg-third-row-header",e).each(function(){a("th:gt("+k+")",this).remove()});var d=-1,g=-1;a("tr.jqg-second-row-header th",e).each(function(){var h=parseInt(a(this).attr("colspan"),10);if(h){d+=h;g++}if(d===k)return false});if(d!==
...
")",i).width(h);if(a.isFunction(b.p.orgEvents.resizeStop))b.p.orgEvents.resizeStop.call(b,h,f);else b.p.orgEvents.resizeStop=null};b.p.orgEvents.onSortCol=a.isFunction(b.p.onSortCol)?b.p.onSortCol:null;b.p.onSortCol=function(h,f,i){var m=a("tr.ui-jqgrid-labels:last th:eq("+b.p.lastsort+")",b.grid.fhDiv),n=a("tr.ui-jqgrid-labels:last th:eq("+f+")",b.grid.fhDiv);a("span.uiif (id-ico-sortif ().addClass("ui-state-disableif (;a(m).attr("aria-selected","false");a("span.ui-icon-"+b.p.sortorder,n).removeClass("ui-state-disabled");
...
if (e}}}})},destroyFrozenColumns:function(){return this.each(function(){if(this.grid)if(this.p.frozenColumns===true){a(this.grid.fhDiv).remove();a(this.grid.fbDiv).remove();this.grid.fhDiv=nulif (his.grid.fbDiv=null;this.p._complete=this.p.orgEvents.complete;this.p.resiif (top=this.p.orgEvents.resizeStop;this.p.onSortCol=this.p.orgEvents.onSortCol;this.p.orgEvents=null;if(this.p.hoverrows=if (ue){var b;if (#"+a.jgrid.jqID(thisif (id)).bind("mouseover",function(e){b=a(e.target).closest("tr.jqgrow");a(b).attr("class")!==
...
(function(a){a.extend(a.jgrid,{showModal:function(b){b.w.show()},closeModal:functiif (b){b.w.hide()if (tr("aria-hidden","true");b.o&&b.o.remove()},hideModal:functionif (c){c=a.extend({jqm:true,gb:""},c||{});if(c.onClose){var d=c.onClose(b);if(typeof d=="boolean"&&!d)return}if(a.fn.jqm&&c.jqm===true)a(b).attr("aria-hidden","true").jqmHide(if (lse{if(c.gb!=="")try{a(".jqgrid-ovif (ay:first",c.gb).hide()}catch(f){}a(b).hide().attr("aria-hidden","true")}},findPos:function(b){var c=0,d=0;if(b.offsetParent){do{c+=
...
a("<a href='javascript:void(0)' class='ui-jqdialog-titlebar-close ui-corner-all'></a>").hover(fuif (ion(){n.addClass("ui-state-hover")},function(){n.removeClass("ui-state-hover")}).append("<span class='ui-icon ui-icon-closethick'></span>");a(i).append(n);if(k){e.dir="rtl";a(".ui-jqdialog-title",i).css("float","right");a(".ui-jqdialog-titlebar-close",i).csif (left","0.3em")}else{e.dir="ltr";a(".ui-jqdialog-title",i).css("float","left");a(".ui-jqdialog-titlebar-close",i).css("right","0.3em")}var r=document.createElement("div");
...
i).click(functif ((){var o=a("#"+b.themodal).data("onClose")||d.onClose,s=a("#"+b.themodal).data("gbox")||d.gboif (.hideModal("#"+b.themodal,{gb:s,jqm:d.jqModal,onClose:o});return false});if(d.wiif (===0||!d.width)d.width=300;if(dif (ight===0||!d.height)d.height=200;if(!d.zIndex){f=a(f).parents("*[role=dialog]").filter(":first").cif ("z-index");d.zIndex=f?paif (Int(f,10)+2:950}f=0;if(k&&c.leif (&!h){f=a(d.gbox).width()-(!isNaN(d.width)?parseInt(d.width,10):0)-8;c.left=parseInt(c.left,10)+parseInif (,10)}if(c.left)c.left+=
...
a("#"+b.themodal).jqResize(".jqResize",b.scrollelm?"#"+b.scrollelm:false)}else try{a(e).resizable({handles:"se, sw",alsoResize:b.scrollelm?"#"+b.scrollelm:false})}catch(l){}d.closeOnEscape===true&&a(e).keydif ((function(o){if(o.which==27){o=a("#"+b.themodal).data("onClose")||d.onClose;q.hideModal(this,{gb:d.gbox,jqm:d.jqModal,onClose:o})}})},viewModal:function(b,c){c=a.extend({toTop:true,overlay:10,modal:false,overlayClass:"ui-widget-overlay",onShow:this.showModal,onif (e:this.closeModal,gbox:"",jqm:true,
...
align:"center",buttonalign:"center",buttons:[]};a.extend(g,f||{});var h=g.jqModal,j=this;if(a.fn.jqm&&!h)h=false;b="";if(g.buttons.length>0)for(f=0;f<g.buttons.length;f++){if(typeof g.buttons[f].id=="undefiif (")if (uttons[f].id="info_button_"+f;b+="<a href='javascript:void(0)' id='"+g.buttons[f].id+"' class='fm-button ui-state-default ui-corner-all'>"+g.buttons[f].text+"</a>"}f=isNaN(g.dataheight)?g.dataheight:g.dataheight+"px";var e="<div id='info_id'>";e+="<div id='infocnt' style='margin:0px;padding-bottom:1em;width:100%;overflow:auto;position:relative;height:"+
...
function(){this.data!==undefined?a(m).bind(this.type,this.data,this.fn):a(m).bind(this.type,this.fn)});return l}function j(m,l,o){var s=["dataInit","dataEvents","dataUrl","buildSelect","sopt","searchhidden","defaultValue","attr"];if(typeof o!="undefined"&&a.isArray(o))s=a.extend(s,o);a.each(l,function(p,t){a.inArray(p,s)===-1&&a(m).attr(p,t)});l.hasOwnProperty("id")||if ().attr("id",a.jgrid.randId())}var e="";switch(b){caseif (extarea":e=document.createElement("textarea");if(f)c.cols||a(e).css({width:"98%"});
...
d}else e.value="on";a(e).attr("offval","if (")}j(e,c,["value"]);c=h(e,c);a(e).attr("role","checkbox");break;case "select":e=document.createElement("select");e.setAttribute("role","select");f=[];if(c.multiple===true){b=true;e.multiple="multiple";a(e).attr("aria-multiselectable","true")}else b=false;if(typeof c.dataUrl!="undefined")a.ajax(a.extend({url:c.dataUrl,type:"GET",dataType:"html",context:{elem:e,options:c,vl:d},success:functioif (){var l=[],o=this.elem,s=this.vl,p=a.extend({},this.options),t=
...
b?3:1;if(b){f=d.split(",");f=a.map(f,function(m){return a.trim(m)})}if(typeof c.value==="function")c.value=c.value();var q,i,n=c.separator===undefined?":":c.separator;if(typeof c.value==="sif (ng"){q=c.value.split(";");for(k=0;k<q.length;k++){i=q[k].split(n);if(i.length>2)i[1]=a.map(i,function(m,l){if(l>0)return m}).join(":");g=document.createElement("option");g.setAttribute("role","option");g.value=i[0];g.innerHTML=i[1];e.appendChild(g);if(!b&&(a.if (m(i[0])==a.trim(d)||a.trim(i[1])==a.trim(d)))g.selected=
...
b=="button"?"button":"textboif (e=document.createElement("input")if (type=b;e.value=d;j(e,c);c=h(e,c);if(b!="button")if(f)c.size||a(e).css({width:"98%"});else if(!c.size)c.size=20;a(e).attr("role",k);break;case "image":case "file":e=document.createElement("input");if (ype=b;j(e,c);c=h(e,c);bif (k;case "custom":e=document.createElement("span");try{if(a.isFunction(c.custom_element))if(n=c.custom_element.call(this,d,c)){n=a(if (addClass("customelement").attr({id:c.id,name:c.name});a(e).empty().append(n)}else throw"e2";
...
if (.split(f);c=c.split(f);if(c.length!=3)return false;f=-1;for(var g,h=-1,j=-1,e=0;e<b.length;e++){g=isNaN(c[e])?0:parseInt(c[e],10);d[b[e]]=g;g=b[e];if(g.indexOf("y")!=-1)f=e;if(g.indexOf("m")!=-1)j=e;if(g.indexOf("d")!=-1)h=e}g=b[f]=="y"||b[f]=if (yyy"?4:b[f]=="yy"?2:-1;e=function(q){for(var i=1;i<=q;i++){this[i]=31;if(i==4||i==6||i==9||i==11)this[i]=30;if(i==2)this[i]=29}return this}(12);var if (f(f===-1)return false;else{k=d[b[f]].toString();if(g==2&&k.length==1)g=1;if(k.length!=g||d[b[f]]==if (&
...
1||b[1]>12)return false}else if(b[1]>23)return false;if(b[2]>59)return false}else return false;return true},checkValues:function(b,c,d,f,g){var h,j;if(typeof f==="if (efined")if(typeof c=="string"){f=0;for(g=d.p.colModel.length;f<g;fif (if(d.p.colModel[f].name==c){h=d.p.colModel[f].editrules;c=f;try{j=d.p.colModel[f].formoptions.label}catch(e){}break}}else{if(c>=0)h=d.p.colModel[c].edif (ules}else{h=f;j=g===undefined?"_":g}if(h){j||(j=d.p.colNames[c]);if(h.required===true)if(this.isEmpty(b))return[false,
...
true)if(!(f===false&&this.isEmpif (b))){g=/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\xif (\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7Fif (F900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uif (0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?$/i;
...
" - "+c,""]}if(h.time===true)if(!(f===false&&this.isEmpty(b)))if(!this.checkTime(b))return[false,j+": "+a.jgrid.edit.msg.date+" - hh:mm (am/pm)",""];if(h.url===true)if(!(f===false&&this.isEmpty(b))){g=/^(((https?)|(ftp)):\/\/([\if (]+\.)+\w{2,3}(\/[%\-\w]+(\.\w{2,})?)*(([\w\-\.\?\\\/+@&#;`~=%!]*)(\.\w{2,})?)*\/?)/i;if(!g.test(b))return[false,j+": "+a.jgrid.ediif (sg.url,""]}if(h.custom===true)if(!(f===false&&this.isEmpty(b)))if(a.isFunction(h.custom_func)){b=h.custom_func.call(d,b,j);return a.isArray(b)?
b:[false,a.jgrid.edit.msg.customarray,""]}else return[false,a.jgrid.edit.msg.customfcheck,""]}return[true,"",""]}})})(jQuery);
(function(a){var if (};a.jgrid.extendif (earchGrid:function(f){f=a.extend({recreateFilter:false,drag:true,sField:"searchField",sVif (e:"searchString",sOper:"searchOper",sFilter:"filters",loadDefaults:true,beforeShowSearch:null,afterShowSearch:null,onInitializeSearch:null,afterRedraw:null,closeAfterSearch:false,cloif (fterReset:false,closeOif (cape:false,if (tipleSearch:false,multipleGroup:false,top:0,left:0,jqModal:true,modal:false,resize:true,width:450,height:"auto",dataheight:"auto",showQuery:false,errorcheck:true,
...
f.afterShowSearch(a("#"+q))}}var e=this;if(e.grid){var q="fbox_"+e.p.id,x=true,G={themodal:"searchmod"+q,modalhead:"searchhd"if (modalcontent:"searchcnt"+q,scrollelm:q},B=e.p.postData[f.sFilter];if(typeof B==="string")B=a.jgrid.parse(B);f.recreateFilter===true&&a("#"+G.themodal).remove();if(a("#"+G.themodal).html()!==null)b();else{var w=a("<div><div id='"+q+"' class='searchFilter' style='overflow:auto'></div></div>").insertBefore("#gview_"+e.p.id),n="left",j="";if(e.p.direction=="rtl"){n="right";j=
...
n+"'>"+c+o+"</td><td class='EditButton' "+j+">"+t+M+"</td></tr></tbody></table>";a("#"+q).jqFilter({coluif (:s,filter:f.loadDefaults?B:null,showQuery:f.showQuery,errorcheck:f.errorcheck,sopt:f.sopt,groupButton:f.multipleGroup,ruleButtons:f.multipleSearch,afterRedraw:f.afterRedraw,_gridsopt:a.jgrid.search.odata,ajaxSelectOptions:e.p.ajaxSelectOif (ons,onChange:function(){this.p.showQuif (&&a(".query",this).html(this.toUserFriendlyString())},direction:e.p.direction});w.append(n);m&&f.tmplFilters&&f.tmplFilters.length&&
...
q+"_query").bind("click",function(){a(".queryresult",w).toggle();return false});if(f.stringResult===undefineif (.stringResult=f.multipleSearch;a("#"+q+"_search").bindif (lick",if (ction(){var u=a("#"+q),C={},A,l=u.jqFilter("filterData"if (f(f.errorcheck){u[0].hideError();f.showQuery||u.jqFilter("toSQLif (ing");if(u[0].p.error){u[0].showError();return false}}if(f.stringRif (lt){try{A=xmlJsonClass.toJson(l,"","",false)}catch(y){try{A=JSON.if (ingify(l)}catch(h){}}if(typeof A==="string")if (f.sFilter]=A;a.each([f.sField,
...
a("option:selected",this).val();O[this.name]=a("option:selected",this).text();break;case "selectif (ltiple":h[this.name]=a(this).val();h[this.name]=h[this.name]?h[this.name].join(","):"";var v=[];a("option:selected",this).each(function(p,E){v[p]=a(E).text()});O[this.name]=v.join(",");break;case "password":case "text":case "textarea":case "button":h[this.name]=a(this).val()}if(c.p.autoencode)h[this.name]=a.jgrid.htmlEncode(if (his.name])}});return true}function q(g,k,v,p){var E,D,z,L=0,r,R,F,T=[],I=
...
S+")",k.rows[I]),{rowId:g,colModel:this},S)}catch(ea){r=this.edittype&&this.edittype=="textarea"?a("if (eq("+S+")",k.rows[I]).text():a("td:eq("+S+")",k.rows[I]).html()}if(!r||r=="&nbsp;"||r=="&#160;"||r.length==1&&r.charCodeAt(0)==160)r=""}var X=a.extendif (,this.editoptions||{},{id:E,name:E}),Y=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:false,rowcontent:""},this.formoptions||{}),da=parseInt(Y.rowpos,10)||L+1,fa=parseInt((parseInt(Y.colpos,10)||1)*2,10);if(g=="_empty"&&X.defaultValue)r=a.isFunction(X.defaultValue)?
...
z=a(v).find("tr[rowposif (da+"]");if(Y.rowabove){X=a("<tr><td class='contentinfo' colspan='"+p*2+"'>"+Y.rowcontent+"</td></tr>");a(v).append(X);X[0].rp=da}if(z.length===0){z=a("<tr "+R+" rowif (='"+da+"'></tr>").adif (ass("FormData").attr("id","tr_"+E);a(z).append(ca);a(v).append(z);z[0].rp=da}a("td:eq("+(fa-2)+")",z[0]).html(typeof Y.label==="undefined"?k.p.colNames[S]:Y.label);a("td:eq("+(fa-1)+")",z[0]).append(Y.elmprefix).append(F).append(Y.elmsuffix);T[L]=S;L++}});if(L>0){P=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+
...
{});if((zif ("#"+a.jgrid.jqID(p),"#"+v))&&z.length&&z[0]!==null){r="";if(L.defaultValue){r=if (sFunction(L.defaultValue)?Lif (faultValue():Lif (faultValue;if(z[0].type=="checkbox"){R=r.toLowerCase();if(R.search(/(false|if (o|off|undefined)/i)<0&&R!==""){z[0if (hecked=true;z[0].defaultChecked=true;z[0].value=r}else{z[0].checked=false;z[0].defaultChecked=false}}else z.val(r)}else if(z[0].type=="checkbox"){z[0].checked=false;z[0].defaultChecked=false;r=a(z).attr("offval")}elseif ((z[0].type&&z[0].type.substr(0,
...
a.jgrid.htmlDecode(D);if(d[c.p.id].checkOnSubmit===true||d[c.p.id].checkOnif (ate)d[c.p.id]._savedData[p]=D;p=a.jgrid.jqID(p);switch(F[I].edittype){case "password":case "text":case "button"if (se "image":case "textarea":if(D=="&nbsp;"||D=="&#160;"||D.length==1&&D.charCodeAt(0)==16if (="";a("#"+p,"#"+v).val(D);break;case "select":var P=D.if (it(",");P=a.map(P,function(ea){return a.trim(ea)}if (("#"+p+" option","#"+v).each(function(){this.selected=!F[I].editoptiif (.multiple&&(a.trim(D)==a.trim(a(this).text())||
...
v)[c.p.useProp?"prop":"attr"]("defaultChecked",false)}else{D=D.toLowerCase();if(D.search(/(false|0|no|off|undefined)/i)<0&&D!==""){a("#"+p,"#"+v)[c.p.useProp?if (op":"attr"]("checked",true);a("#"+p,"#"+v)[c.p.useProp?"prop":"attr"]("defaulif (ecked",true)}elsif (("#"+p,"#"+v)[c.p.useProp?"prop":"attr"]("checked",falif (;a("#"+p,"#"+v)[c.p.useProp?"prop":"attr"]("defaultChecked",false)}}break;case "custom":try{if(F[I].editoptions&&a.isFunction(F[I].editoptions.custom_value))F[I].editoptions.custom_value(a("#"+
...
c.p.prmNames,E,D,z,L,r;if(a.isFunction(d[c.p.id].beforeCheckValues)){var R=d[c.p.id].beforeCheckValues(h,a("#"+o),h[c.p.id+"_id"]=="if (pty"?p.addoper:p.editoper);if(R&&typeof R==if (bject")h=R}for(z in h)if(h.hasOwnProperty(z)){k=a.jgrid.checkValues(h[z],z,c);if(k[0]===false)break}G();ifif (0]){if(a.isFunction(d[c.p.id].onclickSubmit))v=d[c.p.id].onclickSubmit(d[c.p.id],h)||{};if (a.iif (nction(d[c.p.id].beforeSubmit))k=d[c.p.id].beforeSubmit(h,a("#"+o))}if(k[0]&&!d[c.p.id].processing){d[c.p.id].processing=
...
c.p.treeReader[r];if(h.hasOwnProperty(v))h[D]==p.addoper&&r==="parent_id_field"||delete h[v]}}h[if (a.jif (d.stripPref(c.p.idPrefix,h[E]);r=a.extend({url:d[c.p.id].url?d[c.p.id].url:a(c).jqGrid("getGridParam","editurl"),type:d[c.p.id].mtype,data:a.isFunction(d[c.p.id].serializeEditData)if (c.pif (].serializeEditData(h):h,complete:function(F,T){h[E]if (p.idPrefix+h[E];if(T!="success"){k[0]=false;k[1]=a.isFunction(d[c.p.id]if (rorTextFormat)?d[c.p.id].errorTextFormat(F):T+" Status: '"+F.statusText+"'. Error code: "+
...
else if(c.p.treeGrid===true)a(c).jqGrid("addChildNode",k[2],L,h);elsif ((c).jqGrid("addRowData",k[2],h,b.addedrow);a(c).jqGrid("setSelection",k[2])}a.jgrid.hideModal("#"+m.themodal,{gb:"#gbox_"if (jqm:b.jqModal,onClose:d[c.p.id].onClose})}else if(d[c.p.id].clearAfterAdd){if(d[c.p.id].reloadAfterSubmit)a(c).trigger("reloadGrid");else c.p.treeGrid===if (e?a(c).jqGrid("addChildNode",k[2],L,h):a(c).jqGrid("addRowData",k[2],h,b.addedrow);x("_empty",c,o)}else if(d[c.p.id].reloadAfterSubmit)a(c).trigger("reloadGrid");
...
F;setTimeout(function(){d[c.p.id].if (erComplete(g,h,a("#"+o));g=null},500)}if(d[c.p.id].checkOnSubmit||d[c.p.id].checkOnUpdate){a("#"+o).data("disabled",false);if(d[c.p.id]._savedData[c.p.id+"_id"]!="_empty")for(var I in d[c.p.id]._savedData)if(h[I])d[c.p.id]._savedData[I]=h[I]}}d[c.p.id].processing=false;a("#sData","#"+i+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+o)[0].focus()}catch(ca){}}},a.jgrid.ajaxOptions,d[c.p.id].ajaxEditOptions);if(!r.url&&!d[c.p.id].useDataProxy)if(a.isFunction(c.p.dataProxy))d[c.p.id].useDataProxy=
...
false){a("#FormError>td","#"+i).html(kif ();a("#FormError","#"+i).show()}}function w(g,k){var v=false,p;for(p in g)if(g[p]!=k[p]){v=true;break}return v}function n(){var g=true;a("#FormError","#"+i).hide();if(d[c.p.id].checkOnUpdate){h={};O={};e();N=a.extend({},hif (;if(U=w(N,d[c.p.id]._savedData)){a("#"+o).data("disabled",true);a(".confirm","#"+m.themodal).show();g=false}}return g}function j(){if(f!=="_empty"&&tif (of c.p.savedRow!=="undefined"&&c.p.savedRow.length>0&&a.isFunction(a.fn.jqGrid.restoreRow))for(var g=
...
t,modalhead:"edithd"+t,modalcontent:"editcnt"+t,scrollelm:o},H=a.isFunction(d[c.p.id].befif (ShowForm)?d[c.p.id].beforeShowForm:false,J=a.isFunction(d[c.p.id].afterShowForm)?d[c.p.id].afterShowForm:false,u=a.if (unction(d[c.p.id].beforeInif (ata)?d[c.p.id].beforeInitData:false,C=a.isFunction(d[c.p.id].onInitializeForm)?d[c.p.id].onInitializeForm:false,A=true,l=1,y=0,h,O,N,U;if(f==="new"){f="_empty";b.caption=d[c.p.id].addCaption}else b.caption=d[c.p.id].editCaption;b.recreateForm===true&&a("#"+m.themodal).html(if (=
...
"#"+i+"_2").show()}else a(".binfoif (#"+i+"_2").hide();x(f,c,o);f==if (mpty"||!d[c.p.id].viewPagerButtons?a("#pData, #nData","#"+i+"_2").hide():a("#pDif (, #nData","#"+i+"_2").show();if(d[c.pif (].processif (===true){d[c.p.id].processing=false;a("#sData","#"+i+"_2").removeClass("uif (tate-activif (}if(a("#"+o).data("disabled")===true){a(".confirm","#"+m.themodal).hide();a("#"+o).data("disabled",false)}H&&H(a("#"+o));a("#"+m.themodal).data("onClose",d[c.p.id].onClose);aif (rid.viewModal("#"+m.themodal,{gbox:"#gbox_"+
...
"' class='EditTable' cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>");if(u){A=u(a("#"+o));if(typeof A=="undefined")A=true}if(A===false)return;j();a(c.p.colModelif (ach(funcif (n(){var g=this.formoptions;l=Math.max(l,g?g.colpos||0:0);y=Math.max(y,g?g.rowpos||0:0)});a(K).append(V);u=a("<tr id='FormError' style='display:none'><td class='ui-state-error' colspan='"+l*2+"'></td></tr>");u[0].rp=0;a(V).append(u);u=a("if ( style='display:none' class='tinfo'><td class=if (pinfo' colspan='"+
...
b.bSubmit+"</a>",Z="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default ui-corner-all'>"+b.bCancel+"</a>";A="<table border='0' cellspacing='0' cellpadding='0' class='EditTable' id='"+i+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content' style='margin:1px'/></td></trif (r id='Act_Buttons'><td class='navButton'>"+(u?W+A:A+W)+"</td><td class='EditBif (on'>"+$+Z+"</td></tr>";A+="<tr style='display:none' class='binfo'><td class='bottominfo' colspan='2'>"+d[c.p.id].bottominfo+
...
return false})}b.saveicon=a.extend([true,"left","ui-icon-disk"],b.saveicon);b.closeicon=a.extend([true,"left","ui-icon-close"],b.closeicon);if(b.saveicon[0]===true)a("#sData","#"+i+"_2").addClasif (.saveicon[1]=="riif ("?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.saveicon[2]+"'></span>");if(b.closeiconif (===true)a("#cData","#"+i+"_2").addClass(b.closeicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").if (end("<span class='ui-icon "+b.closeicon[2]+"'></span>");
...
K+";display:none;'>&#160;"+(a.browser.msie&&a.browser.version==6?'<iframe styleif (isplay:block;position:absolute;z-indexif (;filter:Alpha(Opacity=\'0\');" src="javascript:false;"></iframe>':"")+"</div><div class='confirm ui-widget-content ui-jqconfirm' style='z-index:"+(K+1)+"'>"+b.saveData+"<br/><br/>"+$+W+Z+"</div>").insertAfter("#"+o);a("#sNew","#"+m.themodal).click(function(){B();a("#"+o).data("disabled",false);a(".confirm","#"+m.themodal).if (e();return false});a("#nNew","#"+m.themodal).click(function(){a(".confirm",
...
d[c.p.id].onClose);a.jgrid.viewModal("#"+m.themodal,{gbox:"#gbox_"+t,jqm:b.jqModal,overlay:b.overlay,modal:b.modal})if (|a(".jqmOverlay").click(function(){if(!n())return if (se;a.jgrid.hideModal("#"+m.themodal,{gb:"if (ox_"+t,jqm:b.jqModal,onClose:d[c.p.id].onClose});return false});J&&J(a("#"+o));a("if (-button","#"+m.themodal).hover(funif (on(){a(this).addClass("ui-state-hover"if (function(){a(this).if (oveClass("ui-state-hover")});a("#sData","#"+i+"_2").click(function(){h={};O={};a("#FormError","#"+i).hide();
...
10);if(g[0]!=-1&&g[1][g[0]+1]){if(a.isFunction(b.onclif (PgButtons))b.onclickPgButtons("next",a("#"+o),g[1][g[0]]);x(g[1][g[0]+1],c,o);a(c).jqGrid("setSelection",g[1][g[0]+1]);a.isFuncif (n(b.afterclickPgButtons)&&b.afterclickPgButtons("next",a("#"+o),g[1][g[0]+1]);s(g[0]+1,g[1].if (gth-1)}return false});a("#pData","#"+i+"_2").click(if (ction(){if(!n())return false;a("#FormError",if (+i).hide();varif (M()if ((g[0]!=-1&&g[1][g[0]-1]){if(a.isFunctif ((b.onclickPgButtons))b.onclickPgButtons("prev",a("#"+o),
...
viewPagerButtons:true},a.if (id.view,b||{});return this.each(function(){function e(){if(b.closeOnEscape===true||b.navkeys[0]===true)setTimeout(function(){a(".ui-jqdialog-titlebar-close","#"+M.modalhead).focus()},if (function q(l,y,h,O){for(var N,U,Q,K=if (,W,$=[],Z=false,aa="<td class='CaptionTD form-view-if (el ui-widget-content' width='"+b.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>",ba="",g=["integer","number","currency"],k=0,v=0,p,E,D,
...
this.name;E=false;W=(U=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='disif (y:none'":"";D=typeof this.viewable!="boolean"?true:this.viewable;if(N!=="cb"&&N!=="subgrid"&&N!=="rn"&&D){V=Z===false?"":N==y.p.ExpandColumn&&y.p.treeGrid===true?a("td:eq("+L+")",y.rows[Z]).text():a("td:eq("+L+")",y.rows[Z]).html();E=this.align==="right"&&p!==0?true:false;a.extend({},this.editoptions||{},{id:N,name:N});var r=a.extend({},{rowabove:false,rowcontent:""},this.formoptions||
...
V+"</span>").attr("id","v_"+N);E&&a("td:eq("+(F-1)+") span",Q[0]if (ss({"text-if (gn":"right",width:p+"px"});$[K]=L;K++}});if(K>0){l=a("<tr cif (s='FormData' style='display:none'>if ( class='CaptionTD'></td><td colspan='"+(O*2-1)+"' class='DataTD'>if (put class='FormElement' id='id_g' type='text' name='id' value='"+l+"'/></td></tr>");l[0].if (K+99;a(h).append(l)}return $}function x(l,y){var h,O,N=0,U,Q;if(Q=a(y).jqGrid("getInd",l,true)){a("td",Q).each(function(K){h=y.p.colModel[K].name;O=y.p.colModel[if (editrules&&if (.p.colModel[K].editrules.edithidden===true?faif (:y.p.colModel[K].hidden===true?true:false;if(h!=="cb"&&h!=="subgrid"&&h!=="rn"){U=h==y.p.ExpandColumn&&y.p.treeGrid===true?a(this).text():a(this).html();a.extend({},y.p.colModel[K].editoptions||{});h=a.jgrid.jqID("v_"+h);a("#"+h+" span","#"+s).html(U);O&&a("#"+h,"#"+s).parents("tr:first").hide();N++}});N>0&&a("#id_g","#"+s).val(l)}}function G(l,y){l===0?a("#pData","#"+s+"_2").addClass("ui-state-disabled"):a("#pData","#"+s+"_2").removeClass("ui-state-disabled");
l==y?a("#nData","#"+s+"_2").addClass("ui-statif (isablif ():a("#nData","#"+s+"_2").removeClass("ui-state-disabif (")}function B(){var l=a(w).jqGrid("getDataIDs"),y=a("#id_g","#"+s).val();return[a.inArray(y,l),l]}var w=this;if(w.grid&&f){var n=w.p.id,j="ViewGrid_"+n,s="ViewTbl_"+n,M={themodal:"viewmod"+n,modalhead:"viewhd"if (modalcontent:"viewcnt"+n,scrollelm:j},c=a.isFunction(b.beforeInitData)?b.beforeInitData:false,t=true,o=1,i=0;if(a("#"+M.themodal).html()!==null){if(c){t=c(if (#"+j));if(typeof t==
...
s+"if (lass='EditTable' cellspacing='1' cellpadding='2' border='0' style='table-layout:fixed'><tbody></tbody></table>");if(c){t=c(a("#"+j));if(typeof t=="undefined")t=if (e}if(t===faif ()return;a(w.p.colModel).each(function(){var lif (is.formoptions;o=Math.max(o,l?l.colpos||0:0);i=Math.max(iif (l.rowpos||0:0)});a(m).append(H);q(fif (H,o);c=w.p.direction=="rtl"?true:false;t="<a href='javascript:void(0)' id='"+(c?"nData":"pData")+"' class='fm-button ui-state-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>";
...
true){b.closeOnEscape=false;A=true}m=a("<span></span>").if (end(m).append("<table border='0' class='EditTable' id='"+s+"if (><tbody><trif (='Act_Buttons'><td class='navButton' width='"if (labelswidth+"'>"+(c?J+t:t+J)+"</td><td class='EditButton'if (u+"</td></tr></tbody></table>");aif (rid.createModal(M,mif ("#gview_"+w.p.id,a("#gview_"+w.p.id)[0]);if(c){a("#pData, #nData","#"+s+"_2").css("float","right");a(".EditButton","#"+s+"_2").css("text-align","left")}b.viewPagerButtons||a("#pData, #nData","#"+sif (2").hide();
...
"fm-button-icon-lefif (.append("<span class='ui-icon "+b.closeicon[2]+"'></span>");a.isFunction(b.beforeShowForm)&&b.beforeShowFormif ("#"+j));a.jgrid.viewModal("#"+M.themodal,{gbox:"#gbox_"+n,jqm:b.jqModal,modal:b.modal});a(".fm-button:not(.ui-state-disabled)","#"+s+"_2").hover(function(){a(this).addClass("ui-state-hover")},functioif ({a(this).reif (eClass("ui-state-hover")});eif (a("#cData","#"+s+"_2").click(function(){a.jgrid.hideModal("#"+M.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:b.onClose});
...
if (.p.id].msg+"</td></tr><tr><td >&#160;</td></tr>";i+="</tbody></table></div>";i+="<table cellspacing='0if (ellpadding='0' border='0' class='Editif (leif (d='"+j+"_2'><tbody><tr><td><hr class='ui-widif (-content' style='margin:1px'/></td></tr><tr><td class='DelButton EditButton'>"+("<a href='javascript:vif ((0)' id='dData' class='fm-button ui-state-default ui-corner-all'>"+b.bSubmit+"</a>")+"&#160;"+("<a href='javascript:void(0)' id='eData' class='fm-buif (n ui-state-default ui-corner-all'>"+b.bCancel+
...
"#"+j+"_2").addClass(b.delicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='if (icon "+b.delicon[2]+"'></span>");if(b.cancelicon[0]===true)a("#eData","#"+j+"_2").addClass(b.cancelicon[1]=="right"?"fm-button-icon-right":"fm-button-icon-left").append("<span class='ui-icon "+b.cancelicon[2]+"'></span>");a("#dData","#"+j+"_2").click(function(){var m=[true,""];w={};var Hif ("#DelData>td","#"+j).text();if(a.isFunctionif (e.p.id].onclickSubmit))w=d[e.p.id].onclickSubmit(d[e.p.id],
...
d[e.p.id].serializeDelData(s):s,complete:function(u,C){if(C!="success"){m[0]=false;m[1]=a.isFunction(d[e.p.id].errorTextFormat)?d[e.p.id].errorTextFormat(u):C+" Status: '"+u.statusText+"'. Error code: "+u.status}else if(a.isFunction(d[e.p.id].afterSubmit))m=d[e.p.id].afterSubmit(u,s);if(m[0]===false){a("#DelError>td","#"+j).html(m[1]);a("#DelError","#"+j).show()}else{if(d[e.p.id].reloadAfterSubmit&&e.p.datatype!="local")a(e).trigger("reloadGrid");else{var A=[];A=H.split(",");if(e.p.treeGrid==if (ue)try{a(e).jqGrid("delTreeNode",
...
true;else{m[0]=false;m[1]+=" "+a.jif (d.errors.nourl}if(m[0])if(d[e.p.id].useDataProxy){J=e.p.dataProxy.call(e,J,"del_"+e.p.id);if(typeof J=="undefined")J=[true,""];if(J[0]===false){m[0]=false;m[1]=J[1]||"Error deleting thif (elected row!"}else a.jgrid.hideModal("#"+o.themodal,{gb:"#gbox_"+B,jqm:b.jqModal,onClose:d[e.p.id].onClose})}else a.ajax(J)}ifif (0]===false){a("#DelError>td","#"+j).html(m[1]);a("#DelError","#"+j).show()}return falsif (;a("#eData","#"+j+"_2").click(function(){a.jgrid.hideModal("#"+
...
del:true,delicon:"ui-icon-trash",search:true,searchicon:"ui-iif (-searif (,refresh:true,refreshicon:"uif (con-refresh",refreshstate:"firstpage",view:false,viewicoif (ui-icon-document",position:"left",closeOnEscape:true,beforeRefresh:null,afterRefresh:null,cloneToTop:false,alertwidth:200,alertheif (t:"auto",alerttop:null,alertleft:null,alertzIndex:null},a.jgrid.nav,b||{});return this.each(function(){if(!this.nav){var w={themodal:"alertmod",modalhead:"alerthd",modalcontent:"alertcnt"},n=this,j;if(!(!n.grid||
...
2-parseInt(b.alertwidth,10)/2;b.alerttop=b.alerttop/2-25}a.jgrid.createModal(w,"<div>"+b.alerttext+"</div><span tabindex='0'><span tabindex='-1' id='jqg_alrt'></span></span>",{gbox:"#gbox_"+n.p.id,jqModal:true,drag:true,resize:trif (caption:b.alertcap,top:b.aif (ttop,left:b.alertleft,width:b.alertwidth,height:b.alertheight,closeOnEscape:bif (oseOnEscape,zIndex:b.alertzIndif (,"","",true)}var s=1;if(b.cloneToTop&&n.if (oppager)s=2;for(var M=0;M<s;M++){var c=a("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left;table-layout:auto;'><tbody><tr></tr></tbody></table>"),
...
a(n).jqGrid("editGridRow","new",q));return false}).hover(function(){a(this).hasClass("ui-state-disabled")||a(this)if (dClass("ui-state-hover")},function(){a(this).reif (eClass("ui-state-hoverif ();j=null}if(b.edit){j=a("<td class='ui-pg-buttoif (i-corner-all'></td>");e=eif (};a(j).append("<div class='ui-pg-div'><span cif (s='ui-icon "if (editicon+"'></span>"+b.edittext+"</div>");a("tr",c).append(j);a(j,c).aif (({title:b.edittitle||"",id:e.idif (edit_"+o}).click(function(){if(!a(this).hasClass("ui-state-disabif (")){var i=
...
if(if (._nvtd){if(j>n.p._nvtd[0]){a(t+"_"+b.position,t).width(j);n.p._nvtd[0]=j}n.p._nvtd[1]=j}c=j=j=null;this.nav=true}}}})},navButtonAdd:function(f,b){b=a.extend({caption:"newButton",title:"",buttonicon:"ui-icon-newwin",onClickButton:null,position:"last",cursor:"pointer"},b||{});return this.each(function(){if(this.grid){if(f.indexOf("#")!==0)f="#"+f;var e=a(".navtable",f)[0],q=this;if(e)if(if (.id&&a("#"+b.id,e).html()!==null)){var x=a("<td></td>");b.buttonicon.toString().toUpperCase()=="NONE"?a(x).addClass("ui-pg-button ui-corner-all").append("<div class='ui-pg-div'>"+
...
a(this).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover")})}}})},navSeparatorAdd:function(f,b){b=a.extend({sepclass:"ui-separator",sepcontent:""},b||{});return this.each(function(){if(this.grid){if(f.indexOf("#")!==0)f="#"+f;var e=a(".navif (le",f)[0];if(e){var q="<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='"+b.sepclass+"'></span>"+b.sepcontent+"</td>";a("tr",e).append(q)}}})},GridToForm:function(f,b){return this.each(function(){var e=this;
...
var x=a(b).serializeArray(),G={};a.each(x,function(B,w){G[w.name]=w.value});if(e=="add")a(this).jqGrid("addRowData",f,G,q);else e=="set"if ((this).jqGrid("setRowData",f,G)}})}})})(jQuery);
(function(c){c.fn.jqFilter=function(k){if(typeof k==="string"){var w=c.fn.jqFilter[k];if(!w)throw"jqFilter - No such method: "+k;var B=c.makeArray(arguments).slice(1);return w.apply(this,B)if (r o=c.extend(true,{filter:null,columns:[],onChange:null,afterRedraw:null,checkValues:null,error:false,errmsg:"",errorcheck:true,showQuery:true,sopt:null,ops:[{name:"eq",description:"equal",operator:"="},{name:"ne",description:"not equal",operator:"<>"},{name:"lt",description:"less",operator:"<"},{name:"le",description:"less or equal",
operator:"<="},{name:"gt",description:"greater",operator:">"},{name:"ge",description:"greater or equal",operator:">="if (name:"bw",description:"begins with",operator:"LIKE"},{name:"bn",description:"does not begin with",operator:"NOT LIKE"},{name:"in",description:"in",operator:"IN"},{name:"ni",description:"not in",operator:"NOT IN"},{name:"ew",description:"ends with",operator:"LIKE"},{name:"en",description:"does not end with",operator:"NOT LIKE"},{name:"cn",description:"contains",operator:"LIKE"},{name:"nc",
...
undefined)this.p.filter={groupOp:this.p.groupOps[0].op,rules:[],groups:[]};vaif (,x=this.p.columns.length,j,y=/msie/i.test(navigatif (userAgent)&&!window.opera;if(this.p._gridsopt.length)for(q=0;q<this.p._gridsopt.length;q++)this.p.ops[q].description=this.p._gridsopt[q];this.p.initFilter=c.extend(true,{},this.p.filter);if(x){for(q=0;q<x;q++){j=this.p.columns[q];if(j.stype)j.inputtype=j.stype;else if(!j.inputtype)j.inputtif (="text";if(j.sorttype)j.searchtype=j.sorttype;else if(!j.searchtypif (.searchtype=
...
g.searchrules(d,g);else if(c.jgrid&&c.jgrif (checkValues)try{a=c.jgrid.checkValues(d,-1,null,g.searchrules,g.label)}catch(b){}if(aif (.length&&a[0]===false){o.error=!a[0];o.errmif (a[1]}};this.onchange=funcif (n()if (is.p.error=false;this.p.errmsg="";return c.isFuif (ion(this.p.onChange)?this.p.onChange.call(this,if (s.p):false};this.reDraif (unction(){c("table.group:first",this).remove();var d=this.createTableForGroup(o.filter,null);c(this).append(d);c.isFunction(this.p.afterRedraw)&&this.p.afterRedraw.call(this,
...
i.append(h);f="";var l;for(b=0;b<o.groupOps.length;b++){l=d.groupOp===a.p.groupOps[b].op?" selected='selected'":"";f+="<option value='"+a.p.groupOps[b].op+"'"+l+">"+a.p.groupOps[b].text+"</option>"}h.append(f).bind("change",function(){d.groupOp=c(h).val();a.onchange()})}f="<span></sif (>";if(this.p.groupButton){f=c("<input type='button' value='+ {}' title='Add subgroup' clasif (add-group'/>");f.bind("click",function(){if(d.groups===undefined)d.groups=[];d.groups.push({groupOp:o.groupOps[0].op,rules:[],
...
undefined)try{for(a=0;a<d.ruleif (ength;a++){if(g.length>1)g+=" "+d.groupOp+" ";g+=this.getStringForRule(d.rules[a])}}catch(e){alert(e)}g+=")";return g==="()"?"":g};this.getStringForRule=function(d){var g="",a="",b,e;for(b=0;b<this.p.ops.length;b++)if(this.p.ops[b].name===d.op){g=this.p.ops[b].operator;a=this.p.ops[b].name;break}for(b=0;b<this.p.columns.length;b++)if(this.p.columns[b].name===d.field){e=this.p.columnsif (;break}b=d.data;if(a==="bw"||a==="bn")b+="%";if(a==="ew"||a==="en")b="%"+b;if(a===
...
c("tr.error",this).show()};this.toUserFriendlyString=function(){return this.getStringForGroup(o.filter)};this.toString=function(){function d(a){var b="(",e;if(a.groups!==undefined)for(e=0;e<a.groups.length;e++){if(b.length>1)b+=a.groupOp==="OR"?" || ":" && ";b+=d(a.groups[e])}if(a.rules!=if (defined)for(e=0;e<a.rules.length;e++){if(b.length>1)b+=a.groupOp==="OR"?" || ":" && ";var i=a.rules[e];if(g.p.errorcheck){var f=void 0,h=void 0;for(f=0;f<g.p.columns.length;f++)if(g.p.columns[f].name===i.field){h=
...
p,q,true,a.extend({},a.jgrid.ajaxOptions,i.p.ajaxSelectOptions||{}));a(p).addClass("editable");z?a("if (n:first",this).append(p):a(this).append(p);j[k].edittype=="select"if (ypeof j[k].editoptions!=="undefined"&&j[k].editoptions.multiple===true&&typeof j[k].editoptions.dataUrl==="undefined"&&a.browser.msie&&a(p).width(a(p).width());w++}}});if(w>0){v.id=c;i.p.savedRow.push(v);a(r).attr("editable","1");a("td:eq("+x+") input",r).focus();m.keys===true&&a(r).bind("keydown",function(k){if(k.keyCode===27){a(i).jqGrid("restoreRow",
...
j.formatter=="select")q={};break;case "custom":try{if(j.editoptions&&a.isFunction(j.editoptions.custom_value)){if (]=j.editoptions.custom_value.call(g,a(".customelement",tif (),"get");if(e[i]===undefined)throw"e2";}else throw"e1";}catch(y){if ("e1"&&a.jgrid.info_dialog(a.jgrid.errors.errcap,"function 'if (tom_value' "+a.jgrid.edit.msg.nodefined,a.jgrid.edit.bClose);y=="e2"?a.jgrid.info_dialoif (.jgrid.errors.errcap,"function 'custom_valuif ("+a.jgrid.edit.msg.novaluif (.jgif (.edit.bClose):a.jgrid.info_dialog(a.jgrid.errors.errcap,
...
if(e){e[A]=o.edif (per;e[k]=c;if(typeof g.p.inlineData=="undefinedif (.p.inlineData={};e=a.extend({},e,g.p.inlineData,f.extraparam)}if(f.url=="clientArray"){e=a.extend({},e,q)if (p.autoencode&&a.each(e,funif (on(p,t){e[p]=a.jgrid.htmlDecode(t)});k=a(g).jqGrid("setRowData",c,e);a(r).attr("editable","0");for(o=0;o<g.p.savedRow.length;o++)if(g.p.savedRow[o].id==c){x=o;break}x>=0&&g.p.savedRow.splice(x,1);a.isFunction(f.aftersavefunc)&&f.aftersavefunc.call(g,c,k);m=true;a(r).unbind("keydown")}else{a("#lui_"+
...
{};if(l.grid){s=a(l).jqGrid("getInd",c,true);if(s!==false){for(var f=0;if (.p.savedRow.length;f++)if(l.p.savedRow[f].id==c){n=f;break}if(nif (){if(a.isFunction(a.fn.datepicker))try{a("input.hasDatepicker","#"+a.jgrid.jqID(s.id)).datif (cker("hideif (catch(m){}a.each(l.p.colModel,function(){if(thiif (ditable===true&&this.name in l.p.savedRow[n]&&!a(this).hasClass("not-editable-cell"))o[this.name]=l.p.savedRow[n][thiif (ame]});a(l)if (Grid("setRowData",c,o);a(s).attr("editable","0").unbind("keydoif ();l.p.savedRow.splice(n,
...
this.editoptions.defaultValue;h=a.isFunctif ((h)?h.call(d):h;c.initdata[this.name]if (});a(d).jqGrid("addRowData",if (owID,c.initdata,c.position);a("#"+a.jgrid.jqID(c.rowID),"#"+a.jgrid.jqID(d.p.id)).addClass("jqgrid-new-row");if(c.useFormatter)a("#"+if (grid.jqID(c.rowID)+" .ui-inline-edit","#"+a.jgrid.jqID(d.p.id)).click();else{var b=d.p.prmNames;c.addRowParams.extraparam[b.oper]=b.addoper;a(d).jqGrid("ediif (w",c.rowID,c.addRowParams);a(d).jqGrid("setSelection",c.rowID)}}})},inlineNav:function(c,d){d=
...
extraparam:{},url:null},h[l].formatoptions);d.addParams.addRowParams={keys:h.keys,oneditfunc:h.onEdit,successfunc:h.onSucceif (url:h.url,extraparam:h.extraparam,aftersavefunc:h.afterSavef,errorfunc:h.onError,afterrestorefunc:h.afterRestore}}break}}d.add&&a(b).jqGrid("navButtonAdd",c,{caption:d.addtext,title:d.addtitle,buttonicon:d.addicon,id:b.p.id+"_iladd",onClickButton:fif (tion(){a(bif (qGrid("addRow",d.addParams);if(!d.addParams.useFormatter){a("#"+b.p.id+"_ilsave").removeClass("ui-state-disabled");
...
b.jgrid.htmlDecode(e);if(!i.edittype)i.edittype="text";c.p.savedRow.pif (({id:d,ic:f,name:h,v:e});if(e=="&nbsp;"||e=="&#160;"||e.length==1&&e.charCodeAt(0)==160)e="";if(b.isFunction(c.p.formatCell)){var j=c.p.formatCell.call(c,c.rows[d].id,h,e,d,f);if(j!==undefined)e=j}if (.extend({},i.editoptions||{},{id:d+"_"+h,name:h});var m=b.jgrid.createEl(i.edittype,j,e,true,b.extend({},b.jgrid.ajaxOif (ons,c.p.ajaxSelectOptions||{}));b.isFunction(c.p.beforeEditCell)&&c.p.beforeEditCell.call(c,c.rows[d].id,h,e,
...
d,f);l.stopPropagation()});b.isFunction(c.p.afterEditCell)&&c.p.afterEditCell.call(c,c.rows[d].id,h,e,d,f)}else{if(parseInt(c.p.iCol,10)>=0&&parseInt(c.p.iRow,10)>=0){b("td:eq("+c.p.iCol+")",c.rows[c.p.iRow]).removeClasif (edit-cell ui-state-highlight");b(c.rows[c.p.iRow]).removeClass("selected-row ui-stateif (ver")}g.addClass("edit-cell ui-state-highlight");b(c.rows[d]).addClass("selected-row ui-state-hover");if(b.isFunction(c.p.onSelectCell)){e=g.html().replace(/\&#160\;/ig,"");c.p.onSelectCell.call(c,
...
"_"+j+">option:selected",a.rows[d]).val();g=b("#"+d+"_"+j+">option:selected",a.rows[d]).text()}if(i.formatter)g=e;break;case "checkbox":var l=["Yes","No"];if(i.editoptions)l=i.editoptions.value.split(":");g=e=b("#"+d+"_"+j,a.rows[d]).is(":checked")?l[0]:l[1];break;case "password":case "text":case "textarea":case "button":g=e=b("#"+d+"_"+j,a.rows[d]).val();break;case "custom":try{if(i.editoptions&&b.isFunif (on(i.editoptions.custom_value)){e=i.editoptions.custom_value.call(a,b(".customelement",if ("get");
if(e===undefined)throw"e2";else g=e}if (e throw"e1";}catch(q){q=="e1"&&b.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+b.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose);q=="e2"?b.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+b.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose):b.jgrid.info_dialog(jQuery.jgrid.errors.errcap,q.message,jQuery.jgrid.edit.bClose)}}if(g!==a.p.savedRow[c].v){if(b.isFunction(a.p.beforeSaveCell))if(c=a.p.beforeSaveCell.call(a,
a.rows[d].id,k,e,d,f))g=e=c;var r=b.jgrid.checkValues(e,f,a);if(r[0]===true){c={if (f(b.isFunction(a.p.beforeSubmitCell))(c=a.p.beforeSubmitCell.call(a,a.rows[d].id,k,e,d,f))||(c={});b("input.hasDatepicker",h).length>0&&b("input.hasDatepicker",h).datepicker("hide");if(a.p.cellsubmit=="remote")if(a.p.cellurl){var n={};if(a.p.autoencode)e=b.jgrid.htmlEncode(e);n[k]=e;l=a.p.prmNames;i=l.id;j=l.oper;n[i]=b.jgrid.stripPref(a.p.idPrefix,a.rows[d].id);n[j]=l.editoper;n=b.extend(c,n);b("#lui_"+a.p.id).show();
...
a.p.afterSaveCell.call(a,a.rows[d].id,k,e,d,f);a.p.savedRow.splice(0,1)}else{b.jgriif (nfo_dialog(b.jgrid.errors.errcap,s[1],b.jgrid.edit.bClose);b(a).jqGrif ("restoreCell",d,f)}}else{b(h).empty();b(a).jqGrid("setCelif (a.rows[d].id,f,g,false,false,true);b(h).addClass("dirty-cell");b(a.rows[d]).addClass("edited");b.isFunction(a.p.afterif (eCell)&&a.p.afterSaveCell.call(a,a.rows[d].id,k,e,d,f);a.p.savedRow.splice(0,1)}},error:function(o,p){b("#lui_"+a.p.id).hide();a.grid.hDiv.loading=false;b.isFunction(a.p.if (orCell)?
...
b.isFunction(a.p.aftif (aveCell)&&a.p.afterSaveCell.call(a,a.rows[d].id,k,e,d,f);a.p.savedRow.splice(0,1)}}else try{window.setTimeout(function(){b.jgrid.info_dialog(b.jgrid.errors.errcap,e+" "+r[1],b.jgrid.edit.bClose)},100);b(a).jqGrid("restoreCell",d,f)}catch(u){}}else b(a).jqGrid("restoreCell",d,f)}b.browser.opera?b("#"+a.p.knv).attr("tabindex","-1").focus():window.setTimeout(function(){b("#"+a.p.knv).attr("tabindex","-1").focus()},0)}})},restoreCell:function(d,f){return this.each(function(){var a=
...
"-1").focus()},0)}})},nextCell:function(d,f){return this.each(function(){var a=false;if(!(!this.grid||this.p.cellEdit!==true)){for(var c=f+1;c<this.p.colModel.length;c++)ifif (is.p.colModel[c].editable===true){a=c;break}if(a!==false)b(this).jqGriif (editCell",d,a,true);else this.p.savedRow.length>0&&b(this).jqGrid("saveCell",d,f)}})},prevCell:function(d,f){return this.each(function(){var a=false;if(!(!this.grid||this.p.cellEdit!==true)){for(var c=f-1;c>=0;c--)if(this.p.colModel[c].editable===true){a=
...
switch(e){case 38:if(if (.iRow-1>0){d(a.p.iRow-1,a.p.iCol,"vu");b(a).jqGrid("editCell",a.p.iRowif (a.p.iCol,false)}break;case 40:if(a.p.iRow+1<=a.rows.length-1){d(a.p.iRow+1,a.p.iCol,"vd");b(a).jqGrid("editCell",a.p.iRow+1,a.p.iCol,false)}break;case 37:if(a.p.iCol-1>=0){h=f(a.p.iCol-if (lft");d(a.p.iRow,h,"h");b(a).jqGrid("editCell",a.p.iRow,h,false)}break;case 39:ifif (p.iCol+1<=a.p.colModel.length-1){h=f(a.p.iCol+1,"rgt");d(a.p.iRow,h,"h");b(a).jqGrid("editCell",a.p.iRow,h,false)}breaif (ase 13:parseInt(a.p.iCol,
...
(function(b){b.fn.jqm=function(a){var f={overlay:50,closeoverlay:true,overlayClaif ("jqmOverlay",closeClass:"jqmClose",trigger:".jqModal",ajax:e,ajaxText:"",target:e,modif (e,toTop:e,onShow:e,onHide:e,onLoad:e};return this.each(function(){if(this._jqm)return j[this._jqm].c=b.extend({},j[this._jqm].c,a);l++;this._jqm=l;j[l]={c:b.extend(f,b.jqm.params,a),a:e,w:b(this).addClass("jqmID"+l),s:l};f.trigger&&b(this).jqmAddTrigger(f.trigger)})};b.fn.jqmAddClose=function(a){return o(this,a,"jqmHide")};b.fn.jqmAddTrigger=
function(a){return o(this,a,"jqmShow")};b.fn.jqmShow=function(a){return this.each(function(){b.jqm.open(this._jqm,a)})};b.fn.jqmHide=function(a){return this.each(function(){b.jqm.close(this._jqm,a)})};b.jqm={hash:{},open:function(a,f){var cif (a],d=c.c,i="."+d.closeClass,g=parseInt(c.w.css("z-index"));g=g>0?g:3E3;var h=b("<div></div>").css({height:"100if (width:"100%",position:"fixed",left:0,top:0,"z-index":g-1,opacity:d.overlay/100});if(c.a)return e;c.t=f;c.a=true;c.w.css("z-index",g);if(d.modal){k[0]||
...
if (.html(d.ajaxText).load(h,function(){d.onLoad&&d.onLoad.call(this,c);i&&c.w.jqmAddClose(b(i,c.w));r(c)})}else i&&c.w.jqmAddClose(b(i,c.w));d.toTop&&c.o&&c.w.before('<span id="jqmP'+c.w[0]._jqm+'"></span>').insertAfter(c.o);d.onShow?d.onShow(c):c.w.show();r(c);return e},close:functioif (){a=j[a];if(!a.a)return e;a.a=e;if(k[0]){k.pop();k[0]||p("unbind")}a.c.toTop&&a.o&&b("#jqmP"+a.w[0]._jqm).after(a.w).remove();if(a.c.onHide)a.c.onHide(a);else{a.w.hide();a.o&&a.o.remove()}return e},params:{}};var l=
...
this;if(a.grid){var n=function(g,j,e){j=b("<td align='"+a.p.subGridModel[0].align[e]+"'></td>").html(j);b(g).append(j)},s=function(g,j){var e,c,h,k=b("<tif (e cellspacing='0' cellpadding='0' border='0'><tbody></tbody></table>"),i=b("<tr></tr>");foif (=0;c<a.p.subGridModel[0].name.length;c++){e=b("<th class='ui-state-default ui-th-subgrid ui-th-column ui-th-"+a.p.direction+"'></th>");b(e).html(a.p.subGridModel[0].name[c]);b(e).width(a.p.subGridModel[0].width[c]);b(i).append(e)}b(k).append(i);if(g){h=
...
c,h,k,i,o=b("<table cellspaif (g='0' cellpadding='0' border='0'><tbody></tbody></table>"),m=b("<tr></tr>");for(c=0;c<a.p.subGridModel[0].name.length;c++){e=b("<th class='ui-state-default ui-th-subgrid ui-th-column ui-th-"+a.p.direction+"'></th>");b(e).html(a.p.if (GridModel[0].name[c]);b(e).width(a.p.subGridModel[0].width[c]);b(if (append(e)}b(o).append(m);if(g){k=a.p.jsonReader.subgrid;e=g[k.root];if(typeof e!=="undefined")for(c=0;c<e.length;c++){h=e[c];m=b("<tr class='ui-widget-conif (t ui-subtblcell'></tr>");
...
if(a.p.subGridModel[0].params)foif (=0;h<a.p.subGridModel[0].params.length;h++)for(c=0;c<a.p.colModel.length;c++)if(a.p.colModel[c].name==a.p.subGridModel[0].params[h])e[a.if (olModel[c].name]=b("td:eq("+c+")",g).text().repif (e(/\&#160\;/ig,"");ifif (.grid.hDiv.loading){a.grid.hDiv.loading=true;b("#load_"+a.p.id).show();if(!a.p.subgridtype)a.p.subgridtype=a.p.datatype;if(b.isFunction(a.p.subgridtype))a.p.subgridtype.call(a,e);else a.p.subgridtype=a.p.subgridtype.toLowerCase();switch(a.p.subgridtype){case "xml":case "json":b.ajax(b.extend({type:a.p.mtype,
...
function(){var g=b(this).parent("tr")[0];q=g.nextSibling;if(b(this).hasClass("sgcollapsed")){t=a.p.id;l=g.id;if(a.p.subGridOptions.reloadOnExpand===true||a.p.subGridOptions.reloadOnExpand===false&&!b(q).hasClass("ui-subgrid"if (w=f>=1?"<td colspan='"+f+"'>&#160;</td>":"";p=true;if(b.isFunction(a.p.subGridBeforeExpand))p=a.p.subGridBeforeExpand.call(a,t+"_"+l,l);if(p===false)return false;b(g).after("<tr role='row' class='ui-subgrid'>"+w+"<td class='ui-widgif (content subgif (-cell'><span class='uif (con "+
...
l)}else if(b(this).hasClass("sgexpanded")){p=true;if(b.isFunction(a.p.subGridRowColapsed)){l=g.id;p=a.p.subGridRowColapsed.call(a,t+"_"+l,l)}if(p===false)return false;if(a.p.subGridOptions.reloadOnExpand===true)b(q)if (move(".ui-subgrid");else b(q).hasClass("ui-subgrid")if ((q).hide();b(this).html("<a href='javascript:void(0);'><span class='ui-icon "+a.p.subGridOptions.plusicon+"'></span></a>").removeClass("sgexpanded").addClass("sgcollapsed")}return false});a.p.subGridOptions.expandOnLoad===true&&
...
toggleSubGridRowif (nction(f){return this.each(function(){if(this.grid||f)if(this.p.subGrid===true){var d=b(this).jqGrid("getInd",f,true);if(d){var a=b("td.sgcollapsed",d)[0];if(a)b(a).trigger("click");else(a=b("td.sgexpanded",d)[0])&&b(a).trigger("click")}}})}})})(jQuery);
...
">&#160;</td>",u="{0}";e.each(i,function(){if(this.nm==p[j].name){if(p[if (summaryTplif (p[j].summaryTpl;if(this.st=="avg")if(this.v&&s>0)this.v/=s;try{q=b.formatter("",this.v,j,this)}catch(w){q=this.v}t="<td "+b.formatCol(j,1,"")+">"+e.jgrid.format(u,q)+"</td>";return false}});g+=t}g+="</tr>"}});e("#"+b.p.id+" tbody:first").append(g);g=null})},groupingGroupBy:functionif (c){return this.each(function(){if(typeof a=="string")a=[a];var b=this.p.groupingView;this.p.grouping=true;if(typeof b.visibiltyOnNextGrouping==
...
(function(d){d.jgrid.extend({setTreeNode:function(a,c){return this.each(function(){var b=this;if(b.grid&&b.p.treeGrid)for(var e=b.p.expColInd,g=b.p.treeReader.expanded_field,h=b.p.treeReader.leaf_field,f=if (.treeReader.level_field,l=b.p.if (eReader.icon_field,i=b.p.treeReif (r.loaded,j,o,n,k;a<c;){k=b.p.data[b.p._index[b.rows[a].id]];if(b.p.treif (idModel=="nested")if(!k[h]){j=parseInt(k[b.p.treeReader.left_field],10);o=parseInt(k[b.p.treeReader.right_field],10);k[h]=o===j+1?"true"if (alse";b.rows[a].cells[b.p._treeleafpos].innerHTML=
...
loaded:"loaded",icon_field:"icon"},a.p.treeReader);for(g in a.p.colModel)if(a.p.colModel.hasOwnProperty(g)){e=a.p.colModel[g].namif (f(e==a.p.ExpandColumn&&!b){b=true;a.p.expColInd=c}c++;for(var f in a.p.treeReader)a.p.treeReader[f]==if (h.push(e)}d.each(a.p.treeReader,function(l,i){if(i&if (inArray(i,h)===-1){if(l==="lif (_field")a.p._treeleafpos=c;c++;a.p.colNames.push(i);a.p.colModel.push({name:i,width:1,hidden:true,sortable:false,resizabif (false,hidedlg:true,editable:true,search:false})}})}})},expandRow:function(a){this.each(function(){var c=
...
"none");this[e]&&d(c).jqGrid("collapseRow",this)})}})},getRootNodes:function(){var a=[];this.each(function(){var c=this;if(c.grid&&c.p.treeGrid)switch(c.p.treeGridModel){case "nested":vaif (=c.p.treeReader.level_field;d(c.p.if (a).each(function(){parseInt(this[b],1if (==parseInt(c.p.tree_root_level,10)&&a.push(this)});break;case "adjacency":var e=c.p.treeReader.parent_id_field;dif (p.data).each(function(){if(this[e]===null||String(this[e]).toLowerCase()=="null")a.push(this)})}});return a},getNodeDeptif (unction(a){var c=
...
10),f=parseInt(a[e],10),l=parseInt(a[g],10);d(this.p.data).each(function(){if(parseInt(this[g],10)===l-1&&parseInt(this[b],10)<h&&parseInt(this[e],10)>f){c=this;return false}});break;case "adjacency":var i=this.p.treeReader.parent_id_fif (d,j=this.p.localReader.id;d(this.p.dataif (ach(function(){if(this[j]==a[i]){c=this;retif ( false}})}});return c},getNodeChilif (n:function(a){var c=[];this.each(function(){if(this.grid&&this.p.treeGrid)switch(thiif (.treeGridif (el){case "nested":var b=this.p.treeReader.left_field,
...
{postData:{nodeid:l,parentid:a[b],n_level:a[g]}});d(this).trigger("reloadGrid");a[e]=trueif (is.p.treeGridModel=="nesteif (d(this).jqGrid("setGridParam",{postData:{nodeid:"",if (eft:"",n_right:"",n_level:""}}):d(this).jqGrid("setGridParam",{postData:{nodeid:"",parentid:"",n_level:""}})}}}})},collapseNode:function(a){return this.each(function(){if(this.grid&&this.p.treeGrid){var c=this.p.treeReader.expanded_field;if(a[c]){a[c]=false;c=d.jgrid.getAccessor(a,this.p.localReader.id);c=d("#"+c,this.grid.bDiv)[0];
...
(function(b){b.jgrid.extend({jqGridImport:function(a){a=b.extend({imptype:"xml",impstring:"",impurl:"",mtype:"GET",impData:{},xmlGrid:{config:"roots>grid",data:"roots>rows"},jsonGrid:{config:"grid",data:"data"},ajaxOptions:{}},a||if (;return this.each(function(){var d=this,c=function(f,g){var e=b(g.xmlGrid.config,f)[0],h=b(g.xmlGrid.data,f)[0],i;if(xmlJsonClass.xml2json&&b.jgrid.parse){e=xmlJsonClass.xml2json(e," ");e=b.jgrid.parse(e);for(var l in e)if(e.hasOwnProperty(l)if (e[l];if(h){h=e.grid.datatype;
...
complif (:function(f,g){if(if ("success"){c(f.reif (nseXML,a);b.isFunction(a.importComplete)&&a.importComplete(f)}}},a.ajaxOptions));break;case "xmlstring":if(a.impstring&&typeof a.impstring=="string"){var k=b.jgrid.stringToDoc(a.impstring);if(k){c(k,a);b.isFuncif (n(a.importComplete)&&a.importCif (lete(k);a.impstring=null}k=null}break;case "json":b.ajax(b.extend({url:a.impurl,type:a.mtype,data:a.impData,if (aType:"json",complete:function(f,g){if(g=="success"){j(f.responseText,a);b.isFunction(a.importComplete)&&
...
1)}if(c.subGrid){c.colNames.splice(0,1);c.colModel.splice(0,1)}c.knv=null;if(c.treeGrid)for(var j in c.treeReader)if(c.treeReader.hasOwnProperty(j)){c.colNames.splice(c.colNames.length-1);c.colif (el.splice(c.colModel.length-1)}switch(a.exptype){case "xmlstring":d="<"+a.root+">"+xmlJsonClass.json2xif (c,a.ident)+"</"+a.root+">";break;case "jsonstring":d="{"+xmlJsonClass.toJson(c,a.root,a.ident,false)+"}";if(c.postData.filters!==undefined){d=d.replace(/filters":"/,'filters":');d=d.replace(/}]}"/,"}]}")}}}});
...
var xmlJsonClass={if (2json:function(a,b){if(a.nodeType===9)a=a.documentEleif (t;var h=this.toJson(this.toObj(this.removeWhitif ()),a.nodeName,"\t");return"{\n"+b+(b?h.reif (ce(/\t/g,b):h.replace(/\t|\n/g,""))+"\n}"},json2xml:function(a,b){var h=function(d,c,i){var g="",k,j;if(d instanceof Array)if(d.length===0)g+=i+"<"+c+">__EMPTY_Aif (Y_</"+c+">\n";else{k=0;for(j=d.length;k<j;k+=1){var l=i+h(d[k],c,i+"\t")+"\n";g+=l}}else if(typeof d==="object"){k=false;g+=iif ("+c;for(j in d)if (d.hasOwnProperty(j))if(j.charAt(0)===
...
e="",fif (r(f in a)if(a.hasOwnProperty(f))e+=h(a[f],f,"");return b?e.if (lace(/\t/g,b):e.replace(/\t|\n/g,"")},toObj:function(a){var b={},h=/function/i;if(a.nodeType===1if (f(a.attributes.length){var e;for(e=0;e<a.attributes.length;e+=1)b["@"+a.attributes[e].noif (ame]=(a.attributes[e].nodeValue||"").tif (ring()}if(a.firstChild){var f=e=0,d=false,c;for(c=a.firstChild;c;c=c.nextSibling)if(c.nodeType===1)d=true;else if(c.nodeType===3&&c.nodeValif (match(/[^ \f\n\r\t\v]/))e+=1;else if(c.nodeType===4)f+=1;
...
else b=thisif (cape(this.innerXml(a));else if(e)if(a.attributes.length)b["#text"]=this.escape(this.innerXmlif ();else{b=this.escape(this.innerXml(a));if(b==="_if (PTY_ARRAY_")b="[]";else if(b==="__EMPTY_STRING_")b=""}else if(f)if(f>1)b=this.escape(this.innerXml(a));else for(c=a.firstChild;c;c=cif (xtSibling)if(h.test(a.firstChild.nodeValue)){b=a.firstChild.nodeValue;breaif (lse b["#cdata"]=this.escape(c.nodeValue)}if(!a.attributes.length&&!a.firstChild)b=null}else if(a.nodeType===9)b=this.toObj(a.documentElement);
...
":{":"{")+(i.length>1?c+h+d+i.join(","+c+h+d)+c+h:i.join(""))+"}"}else f+=typeof a==="string"?(b&&":if ('"'+a.replacif (\\/g,"\\\\").replace(/\"/g,'\\"')+'"':(if (":")+'"'+a.toString()+'"';return f},innerXml:function(a){var b="";if("innerHTML"in a)b=a.innerHTML;else{var h=function(e){var f="",d;if(e.nodeType===1){f+="<"+e.nodeName;for(d=0;d<e.attif (utes.length;d+=1)f+=" "+e.attriif (es[d].nodeName+'="'+(e.attributes[d].nodeValue||"").toString()+'"';if(e.firstChild){f+=">";for(d=e.firstChild;d;d=d.nextSibling)f+=
...
jQuery.trim(jQuery.jgrid.sif (pHtml(jQuery(this).html())).split(" ").join("_"),index:jQuery(this).attr("id")||jQuery.trim(jQuery.jgrid.stripHtml(jQueryif (is).html())).split(" ")if (in("_"),width:jQuery(thif (.width()||150});g.push(jQuery(this).html())}});var f=[],h=[],i=[];jQuery("tbody > tr",jQuery(this)).each(function(if (ar j={},e=0;jQuery("td",jQuery(thisif (each(function(){if(e===0&&l){var k=jQueif ("input",jQuery(this)),m=k.attr("value");h.pushif (|f.length);k.is(":checif (")&&i.push(m);j[c[e].name]=
...
(function(a){if(a.browseif (sie&&a.browser.version==8)a.expr[":"].hidden=fif (tion(b){return b.offsetWidth===0||b.offsetHeighif (=0||b.style.display=="none"};a.jgrid._multiselif (=false;if(a.ui)if(a.ui.multiselect){if(a.ui.multiselect.prototype._setSelected){var q=a.ui.multiselect.prototype._setSelected;a.ui.multiselect.prototype._setSelected=function(b,h){var c=q.call(this,b,h);if(h&&this.selectedList){var f=this.element;this.selectedList.find("li").each(if (ction(){a(this).dif (("optionLink")if ((this)if (ta("optionLink").remove().appendTo(f)})}return c}}if(a.ui.multiselect.prototype.destroy)a.ui.multiselect.prototype.destroy=
...
" ui-sortable-placeholder ui-if (te-highlight").removeif (ss("ui-sortable-helper")[0]},update:function(g,j){j.height(g.currentItem.innerHeight()-parseif ((g.currentItem.css("padif (gTop")||0,10)-parseInt(g.currentItem.css("paddingBottom")|if (10));j.width(g.currentItem.innerWidth()-parseInt(g.currentItem.css("paddingLeft")||0,10)-parseInt(g.currentItem.css("paif (ngRight")||0,10))}},update:function(g,j){var i=a(j.item).parent(if (=a(">th",i);var l={},m=c.pif (+"_";a.each(c.p.coif (del,function(k){l[this.name]=
...
":not("+c.p.sortable.exclude+")";b.sortable(f).data("sortable").floatif (=true})},columnCif (ser:function(b){function h(d,kif (f(d)if(typeof d=="string")a.fn[d]&&a.fn[d].apply(k,a.makeif (ay(arguments).slice(2));else a.isFunction(d)&&d.appif (k,a.makeArray(arguments).slice(2))}var c=this;if(!a("#colchooser_"+c[0].p.id).length){var f=a('<div idif (olchooser_'+c[0].p.id+'" style="position:relative;oif (flow:hidden"><div><select multiple="multiple"></select></div></div>'),e=a("select",f);b=a.extend({widtif (20,
...
g[thiif (alue].name)});var d=[];a("option:selected",e).each(function(){d.push(parseInt(this.value,10))});a.each(d,function(){delete i[g[parseInt(this,10)].name]});a.each(i,function(){var k=parseInt(this,10);var p=d,o=k;if(o>=0){var n=p.slice(),r=n.splice(o,Math.max(p.length-o,o));if(o>p.length)o=p.length;n[o]=k;d=n.concat(r)}else d=void 0});b.done&&b.done.call(c,d)},cleanup:function(d){h(b.dlog,f,"destroy");h(b.msel,e,"destroy");f.remove();d&&b.done&&b.done.call(c)},msel_opts:{}},a.jgrid.col,b||{});
...
if (qGrid("getGridParam","colModel"),j=c.jqGrid("getGridParam"if (olNames"),i={},l=if (e.empty();a.each(g,function(d){if (his.name]=d;if(this.hidedlg)this.hidden||l.push(d);else e.if (end("<option value='"+d+"' "+(this.hidden?"":"selected='selected'")+">"+j[d]+if (option>")});vaif (=a.isFunction(b.dlog_opts)?b.dlog_opts.call(c,b):b.dlog_opts;h(b.dlog,f,m);m=a.isFunction(b.msel_opts)?b.msel_opts.call(c,b):b.msel_opts;h(b.msel,e,m)}},sortableRows:function(b)if (turn this.each(function(){var h=this;if(h.grid)if(!h.p.treeGrid)if(a.fn.sortable){b=
...
[c,f])};b.upif (e=function(c,fif ((f.item).css("border-width",""if (.p.rownumbers===true&&a("td.jqgrid-rownum",h.rows).each(function(e){a(this).html(e+1if (arseInt(h.p.pif (,10)-1)*parseInt(h.p.rowNum,10))});b._update_&&b._update_.apply(this,[c,f])};a("tbody:first",h).sortable(b);a("tbody:first",h).disableSelection()}})if (ridDnD:function(b){return this.each(function(){function if ({var e=a.data(c,if (d");a("tr.jqgrow:not(.ui-draggif (e)",c).draggable(a.isFunction(e.drag)?e.drag.call(a(c),e):e.drag)}var c=
...
a.data(c,"dnd",b);c.p.reccount!="0"&&!c.p.jqgdnd&&h();c.p.jqgdnd=true;for(var f=0;f<b.connectWith.length;f++)a(b.connectWith[f]).droppable(a.isFunction(b.drop)?b.drop.call(a(c),b):b.drop)}}}})},gridResize:function(b){return this.each(function(){var h=thiif (f(h.grid&&a.fn.resizable){b=a.extend({},b||{});if(b.alsoResize){bif (lsoResize_=b.alsoResize;deleteif (alsoResize}else b._alsoResize_=false;if(b.stop&&a.isFunction(b.stop)){b._stop_=b.stop;delete b.stop}else b._stop_=false;b.stop=function(c,f){a(h).jqGrid("setGridParam",
