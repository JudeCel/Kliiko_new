(funiif ((((((f (( (if (((( (((( ( ( (( ( ( ((( ((f ((( ( ( (( ( (( (( ((((( ((( (((f ( ( ( ( (((( ( ( (iif (( ( (((if ((( (((if (onif ({
/**
...
$.jgrid.extend({
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if (.each(p.tmplNames, function(i,n){
				...
				if (f(this.p.showQuery) {
							$('.query',this).html(this.toUserFriendlyString());
						}
					},
					if (ection : $t.p.direction
				});
				fil.append( bt );
				...
				p.gbox = "#gbox_"+fid;
				if (p.layer) {
				if (jgrid.createModal(IDs ,fil,p,"#gview_"+$t.p.id,$("#gbox_"+$t.p.id)[0], "#"+p.layer, {position: "relative"});
				} else {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if (rowid === "new") {
				...
				p.if (tion=rp_ge[$t.p.id].editCaption;
			}
			...
			function getFormData(){
				...
				if (		if (postdata[nm] === undefined) {throw "e1";}
								} catch (e) {
				...
				return true;
			}if (	function createData(rowid,obj,tb,maxcols){
				var nm, hc,trdata, cnt=0,tmp, dc,elc, retpos=[], ind=false,
				tdtmpl = "<td class='CaptionTD'>&#160;</td><td class='DataTD'>&#160;</td>", tmpl="", i; //*2
				foif (i =1; i<=maxcols;i++) {
					tmpl += tdtmpl;
				}if (		if(rowid != '_empty') {
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if (opt = $.extend({}, {elmprefix:'',elmsuffix:'',rowabove:false,rowcontent:''}, this.formoptions || {}),
			...
			fuif (ion fillData(rowid,obj,fmid){
			...
			if (	} else {
							...
							if(ret[0] === false) {
			if (		$("#FormError>td","#"+frmtb).html(ret[1]);
			...
			function updateNav(cr,totr){
				if if (===0) {$("#pData","#"+frmtb+"_2").addClass('ui-state-disabled');} else {$("#pData","#"+frmtb+"_2").removeClass('ui-state-disabled');}
	...
	if (if(showFrm === false) {return;}
				...
				///
				ifif (wid=="_empty" || !rp_ge[$t.p.id].viewPagerButtons) {
					$("#pData, #nData","#"+frmtb+"_2").hide();
				} if (e {
					$("#pData, #nData","#"+frmtb+"_2").show();
				}
				if(rp_ge[$t.p.id].processing===true) {
					rif (e[$t.p.id].processing=false;
					$("#sData", "#"+frmtb+"_2").removeClass('ui-state-active');
				}if (		if($("#"+frmgr).data("disabled")===true) {
					$if (confirm","#"+IDs.themodal).hide();
					$("#"+frmgr).data("disabled",false);
				}
				if(onBeforeShow) {onBeforeShow($("#"+frmgr));}
				$(if (+IDs.themodal).data("onClose",rp_ge[$t.p.id].onClose);
				...
				}if (		if(onAfterShow) {onAfterShow($("#"+frmgr));}
			} else {
				...
				ifif (_ge[$t.p.id].topinfo) {$(".tinfo","#"+frmtb).show();}
				...
				$("#"+IDs.themodal).keydown( function( e ) {
					...
					if(rp_ge[$t.p.id].savekey[0] === true && e.which == rp_ge[$t.p.id].savekey[1]) { // save
						if(wkey.tagName != "TEXTAREA") {
							$if (sData", "#"+frmtb+"_2").trigger("click");
							return false;
						}
					}
					if(eif (ich === 27) {
				...
				if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
					...
					bCif ("<a href='javascript:void(0)' id='cNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+p.bExit+"</a>";
				...
				$(".fm-button","#"+IDs.themodal).hover(
				   function(){$(this).addClass('ui-state-hover');},
				   function(){$(this).removeClass('ui-state-hover');}
				);if (			$("#sData", "#"+frmtb+"_2").click(function(e){
					pif (data = {};extpost={};
					$("#FormError","#"+frmtb).hide();
				...
				$("#nData", "#"+frmtb+"_2").click(function(e){
					...
					if(npos[0] != -1 && npos[1][npos[0]+1]) {
						...
						fillif (a(npos[1][npos[0]+1],$t,frmgr);
			...
			updateif ((posInit[0],posInit[1].length-1);
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				var nif (hc,trdata, cnt=0,tmp, dc, retpos=[], ind=false,
				...
				ind = $(obj).jqGrid("getInd",rowid);
				if (bj.p.colModel).each( function(i) {
					nm = this.name;
					setme = false;
					// hidden fields are included in the form
					if(this.editrules && this.editrules.edithidden === true) {
						...
						hc = this.hidden === true ? true : false;
					if (					dc = hc ? "style='display:none'" : "";
					vif (fld = (typeof this.viewable != 'boolean') ? true : this.viewable;
				...
				if( cnt > 0) {
				if (r idrow = $("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+ (maxcols*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+rowid+"'/></td></tr>");
					idrow[0].rp = cnt+99;
					if (b).append(idrow);
				}
				return retpos;
			...
			function fillData(rowid,obj){
				...
				tif (= $(obj).jqGrid("getInd",rowid,true);
				if(!trv) {return;}
			...
			function updateNav(cr,totr){
				...
				ifif (r==totr) {$("#nData","#"+frmtb+"_2").addClass('ui-state-disabled');} else {$("#nData","#"+frmtb+"_2").removeClass('ui-state-disabled');}
			}
			function getCurrPos() {
				var rowsInGrid = $($t).jqGrid("getDataIDs"),
				selrow = $("#id_g","#"+frmtb).val(),
			...
			if (( $("#"+IDs.themodal).html() !== null ) {
				...
				tblif (("<table id='"+frmtb+"' class='EditTable' cellspacing='1' cellpadding='2' border='0' style='table-layout:fixed'><tbody></tbody></table>");
				if(onBeforeInit) {
					shoif (m = onBeforeInit($("#"+frmgr));
					if(typeof(showFrm) == "undefined") {
						showFrm = true;
					}
				}
				...
				if(!if (iewPagerButtons) {$("#pData, #nData","#"+frmtb+"_2").hide();}
				bt = null;
				$("#"+IDs.themodal).keydown( function( e ) {
					if(e.which === 27) {
				...
				p.closeicon = $.extend([true,"left","ui-icon-close"],p.closeicon);
				if(p.closeicon[0]===true) {
					$("#if (ta","#"+frmtb+"_2").addClass(p.closeicon[1] == "right" ? 'fm-button-icon-right' : 'fm-button-icon-left')
					.append("<span class='ui-icon "+p.closeicon[2]+"'></span>");
				}
				if($.isFunction(p.beforeShowForm)) {p.beforeShowForm($("#"+frmgr));}
				...
				focusaref();
				$("#if (ta", "#"+frmtb+"_2").click(function(e){
				...
				$("#nData", "#"+frmtb+"_2").click(function(e){
					...
					if(npos[0] != -1 && npos[1][npos[0]+1]) {
						...
						$($t).jqGrid("setSelection",npos[1][npos[0]+1]);
						ifif (isFunction(p.afterclickPgButtons)) {
							p.afterclickPgButtons('next',$("#"+frmgr),npos[1][npos[0]+1]);
						}
						updateNav(npos[0]+1,npos[1].length-1);
					}
				...
				$("#pData", "#"+frmtb+"_2").click(function(e){
					...
					var if (s = getCurrPos();
					if(ppos[0] != -1 && ppos[1][ppos[0]-1]) {
						if($if (Function(p.onclickPgButtons)) {
							p.onclickPgButtons('prev',$("#"+frmgr),ppos[1][ppos[0]]);
						}
						fillData(ppos[1][ppos[0]-1],$t);
						$($t)if (Grid("setSelection",ppos[1][ppos[0]-1]);
						if($.isFunction(p.afterclickPgButtons)) {
							p.afterclickPgButtons('prev',$("#"+frmgr),ppos[1][ppos[0]-1]);
						}
						updateNav(ppos[0]-1,ppos[1].length-1);
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ((showFrm === false) {return;}
				...
				$("#DelError","#"+dtbl).hide();
				if( rpif ([$t.p.id].processing === true) {
					rp_ge[$t.p.id].processing=false;
				...
				// error data
				tbl +if (<tr id='DelError' style='display:none'><td class='ui-state-error'></td></tr>";
				tbl += "<tr id='DelData' style='display:none'><td >"+rowids+"</td></tr>";
				tbl += "<tr><td class=\"delmsg\" style=\"white-space:pre;\">"+rp_ge[$t.p.id].msg+"</td></tr><tr><td >&#160;</td></tr>";
				// buttons at footer
				tbl if ("</tbody></table></div>";
	...
	if (if(showFrm === false) {return;}
				...
				$(".fm-button","#"+dtbl+"_2").hover(
				   funif (on(){$(this).addClass('ui-state-hover');},
				   function(){$(this).removeClass('ui-state-hover');}
				);
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						var ajaxOptions = $.extend({
							...
							if (
								...
								if (for(var i=0;i<toarr.length;i++) {
				...
				if (f (!ajaxOptions.url && !rp_ge[$t.p.id].useDataProxy) {
							if ($.isFunction($t.p.dataProxy)) {
								rp_ge[$t.p.id].useDataProxy = true;
							} else {
								ret[0]=false;ret[1] += " "+$.jgrid.errors.nourl;
			...
			iif (p_ge[$t.p.id].closeOnEscape===true) {
				setTimeout(function(){$(".ui-jqdialog-titlebar-close","#"+IDs.modalhead).focus();},0);
			}
		});
	},
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				if (o.add) {
					...
					if (bd).append("<div class='ui-pg-div'><span class='ui-icon "+o.addicon+"'></span>"+o.addtext+"</div>");
				...
				if (} else {
						...
						function () {
							if (!$(this).hasClass('ui-state-disabled')) {
				if (	$(this).addClass("ui-state-hover");
							}
						},
						function () {$(this).removeClass("ui-state-hover");}
					);
				...
				if (o.view) {
				if (d = $("<td class='ui-pg-button ui-corner-all'></td>");
					pView = pView || {};
				...
				if (	$.jgrid.viewModal("#"+alertIDs.themodal,{gbox:"#gbox_"+$t.p.id,jqm:true});
								$("#jqg_alrt").focus();
							}
						}
						return false;
				...
				if (if($.isFunction(o.afterRefresh)) {o.afterRefresh();}
						}
						return false;
					}).hover(
						function () {
					...
					tbd = null;
				}if (		tdw = $(".ui-jqgrid").css("font-size") || "11px";
				$(if (dy').append("<div id='testpg2' class='ui-jqgrid ui-widget ui-widget-content' style='font-size:"+tdw+";visibility:hidden;' ></div>");
	...
	navSeif (atorAdd:function (elem,p) {
		p = $.extend({
			sepclass : "ui-separator",
			sepcontent: ''
		}, p ||{});
		return this.each(function() {
			ifif (this.grid)  {return;}
			if( elem.indexOf("#") !== 0) {elem = "#"+elem;}
			varif (ndnav = $(".navtable",elem)[0];
			if(findnav) {
				vaif (ep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='"+p.sepclass+"'></span>"+p.sepcontent+"</td>";
	...
	FormTif (id : function(rowid, formid, mode, position){
		return this.each(function() {
			var $t = this;
			if(!$t.grid) {return;}
			if(!mode) {mode = 'set';}
