...
if ($.ui.multiselect.prototype.destroy = function() {
				this.element.show();
				this.container.remove();
				if ($.Widget === undefined) {
		if ($.widget.prototype.destroy.apply(this, arguments);
				} else {
...
$.jgrid.extend({
	sortableColumns : function (tblrow)
	{
		...
		if (	p.width(self.currentItem.innerWidth() - parseInt(self.currentItem.css('paddingLeft')||0, 10) - parseInt(self.currentItem.css('paddingRight')||0, 10));
					}
				},
  ...
  if (   }
   ...
   if (  $.each(colModel, function(i) {
	...
	gridDnD : function(opts) {
		...
		if (	if(ui.helper.dropped) {
							var grid;
							if(opts.autoid) {
								if($.isFunction(opts.autoid)) {
				if (		grid = opts.autoid.call(this,getdata);
								} else {
					if (	grid = Math.ceil(Math.random()*1000);
