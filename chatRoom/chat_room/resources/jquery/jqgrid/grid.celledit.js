	...
	saveCell : function (iRow, iCol){
		return this.each(function(){
			...
			if(fr !== null) {
				...
				if (var sel = $("#"+iRow+"_"+nmjq,$t.rows[iRow]), selectedText = [];
							v = $(sel).val();
							if(v) { v.join(",");} else { v=""; }
				...
				if (v2 = selectedText.join(",");
					...
					case 'custom' :
						...
						if ( (e=="e1") { $.jgrid.info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+$.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose); }
							...
							if (e {$.jgrid.info_dialog(jQuery.jgrid.errors.errcap,e.message,jQuery.jgrid.edit.bClose); }
						}
						break;
				}
				...
				if (v2 !== $t.p.savedRow[fr].v){
					...
					if(cv[0] === true) {
						...
						if ($.isFunction($t.p.beforeSubmitCell)) {
							...
							if (!addpost) {addpost={};}
						if (						if( $("input.hasDatepicker",cc).length >0) { $("input.hasDatepicker",cc).datepicker('hide'); }
			...
			if (		try {
						...
						if ($t.p.cellsubmit == 'clientArray') {
							...
							$($t).jqGrid("setCell",$t.rows[iRow].id,iCol, v2, false, false, true);
							$(cc).addClass("dirty-cell");
						if ($t.rows[iRow]).addClass("edited");
							if ($.isFunction($t.p.afterSaveCell)) {
								$t.p.afterSaveCell.call($t, $t.rows[iRow].id,nm, v, iRow,iCol);
							}
							$t.p.savedRow.splice(0,1);
		...
		return this.each(function () {
			...
			.keydown(function (e){
				...
				switch (kdir) {
					...
					break;
					case 39 :
						ifif (t.p.iCol +1 <=  $t.p.colModel.length-1) {
