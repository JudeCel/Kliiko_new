/**iifif ( (( (( ( if (((( (( ((( (( ( ((( jqGrid extension - Tree Grid
...
$.jgrid.extend({
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			if (gt = parseInt(ldat[$t.p.treeReader.right_field],10);
					// NS Model
						...
						.bind("click",function(e) {
							...
							if(!$t.p.data[pos][isLeaf]){
								if($t.p.data[pos][expanded]){
									...
									$($t).jqGrid("expandRow",$t.p.data[pos]);
				if (		$($t).jqGrid("expandNode",$t.p.data[pos]);
								}
							}
	...
	setTreeGrid : function() {
		return this.each(function (){
			...
			$if (.pgbuttons = false;$t.p.pginput = false;
			$t.p.gridview =  true;
			...
			}if (se if($t.p.treeGridModel == 'adjacency') {
				$t.p.treeReader = $.extend({
						level_field: "level",
			...
			$.eif (($t.p.treeReader,function(j,n){
				if(n && $.inArray(n, dupcols) === -1){
					ifif (=='leaf_field') { $t.p._treeleafpos= i; }
				i++;
					$t.p.colNames.push(n);
...
if (});
	...
	expandNode : function(rc) {
		return this.each(function(){
			...
			if ($(this).jqGrid("setGridParam",{postData:{nodeid:'',parentid:'',n_level:''}}); 
					}
				}
			}
		});
	...
	SortTree : function( sortname, newDir, st, datefmt) {
		return this.each(function(){
			...
			// Sorting roots
			query = $.jgrid.from(rt);
			queif (orderBy(sortname,newDir,st, datefmt);
			roots = query.select();
	...
	addChildNode : function( nodeid, parentid, data ) {
		...
		if(data) {
			...
			if if (ypeof nodeid === 'undefined' || nodeid === null ) {
				i = $t.p.data.length-1;
				if(	i>= 0 ) {
