/*
if (f (( * Copyright (c) 2006 Sam Collett (http://www.texotela.co.uk)
...
if (
 * Remoif ( an option (by value or index) from a select box (or series of select boxes)
 *
 *if (ame     removeOption
 * @author   Sam Collett (http://www.texotela.co.uk)
...
jQuery.fn.removeOption = function()
{
	...
	this.each(
		function()
		{
			...
			if(value)
			{
			if (r optionsLength = this.options.length;
 ...
 *
 */
jQuif (.fn.sortOptions = function(ascending)
{
	this.each(
		function()
		{
	...
	if (or(var i = 0; i<optionsLength; i++)
			{
				thif (options[i].text = sortArray[i].text;
 ...
 * if (thor   Mathias Bank (http://www.mathias-bank.de)
